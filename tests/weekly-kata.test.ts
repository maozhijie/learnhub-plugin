...
import { todayStr, dayOfTs } from '../src/engine/infra/dates.ts'
...
// ---- #316 罗盘周检讨旁挂：覆盖缺口/深度差/配比差三类读数进现状区，只摆读数不触发重画 ----

test('#316 周检讨渲染：覆盖缺口 + 深度差 + 配比逐行；零读数不出告警；空注入零小节', () => {
  const reality = buildKataReality({
    weekStart: '2026-09-07', weekEnd: '2026-09-13', cutoffMin: 0,
    practice: [], journal: [], reviewLog: [], habitRepeats: [],
    projects: [], projectExec: {}, noteSources: {}, habitNames: {}, skillNames: {},
  ...
  const reviews = [{
    course: '数学', endpoint: '用导数解决优化问题',
    gaps: ['几何补面'],
    depth_gaps: [{ name: '认识变化率', required: '会用' as const, actual: '知道' as const }],
    shares: [{ name: '认识变化率', nodes: 3 }, { name: '几何补面', nodes: 0 }],
  }]
  const md = renderKataReality(reality, etas, reviews)
  assert.ok(md.includes('### 罗盘周检讨'))
  assert.ok(md.includes('覆盖缺口——「几何补面」在图上还没有节点'))
  assert.ok(md.includes('深度差——「认识变化率」需求 会用 / 现状 知道'))
  assert.ok(md.includes('跨面配比（粗）——「认识变化率」3 节点'))
  assert.ok(md.includes('只摆读数，不触发重画'), '措辞沿非权威纪律（读数高频免费，改弧低频留痕）')
  assert.ok(md.indexOf('### 沙盘 ETA') < md.indexOf('### 罗盘周检讨'), '周检讨旁挂在沙盘 ETA 之后（同一挂载点顺带）')
  assert.ok(md.indexOf('### 罗盘周检讨') < md.indexOf('### 有界 · 项目'))

  assert.doesNotMatch(renderKataReality(reality), /### 罗盘周检讨/, '空注入 = 零小节（未画路线的课不出读数）')
})
