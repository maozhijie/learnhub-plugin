...
test('路由级：pair 提案 apply 自动走联合入口，plan 半区的生长触发随后入队（#156）', async () => {
  ...
  project.projectDecompileApply = async (...args: unknown[]) => {
    ...
    return { project: '反编译项目', seed: {}, plan: { kind: 'project_plan', growth: [{ course: '线性代数', lines: ['补支'] }] } }
  }
...
test('路由级：无 pair 的提案 apply 走单边路径（引擎调用序列不变）', async () => {
  ...
  graph.proposalApply = async (...args: unknown[]) => { calls.push(`apply(${args.join(',')})`); return { kind: 'seed' } }
