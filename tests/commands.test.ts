...
test('门④ handler 覆盖：handlers.ts 的键集合 == 没有 bind 的路由通道（panel+ops）集合', () => {
  ...
  assert.equal(generic.length + handled.length, 127, '路由通道（panel+ops）总数应恰 127（#209 +1：GET /compass；#215 +1：POST /smoke；#216 +1：POST /spike；#222 +1：POST /quality-review；#240 +3：POST /course/create、POST /endpoint/add、POST /endpoint/remove；#248 +1：POST /coach/stuck-report；#255 −1：GET /doctor；#256 −6：六条死路由退役）')
