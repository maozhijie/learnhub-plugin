...
import { LearnerSubsystem } from '../src/engine/learner/learner-cards.ts'
import { ContentSubsystem } from '../src/engine/content/content-subsystem.ts'
import { ProjectSubsystem, Projects } from '../src/engine/practice/projects.ts'
import { BankSubsystem } from '../src/engine/content/question-bank.ts'
import { ChannelsSubsystem } from '../src/engine/vault/note-source.ts'
import { LabSubsystem } from '../src/engine/coach/nof1.ts'
import { GraphSubsystem } from '../src/engine/graph/graph-subsystem.ts'
import { GrowthSubsystem } from '../src/engine/coach/growth-subsystem.ts'
import { SchedSubsystem } from '../src/engine/sched/sched-subsystem.ts'
import { Registry } from '../src/engine/vault/registry.ts'
import { GraphProposals } from '../src/engine/coach/proposals.ts'
...
test('门③ 唯一性：id／tool 名／(method, path) 各自唯一，索引没被静默覆盖', () => {
  ...
  assert.equal(tools.length, 117, `agent 通道应恰 117 条，实得 ${tools.length}（#313 +3：learnhub_growth_batch / learnhub_generate_cancel / learnhub_generate_status；#312 +1：learnhub_coach_draft_cancel（草稿逃生口）；#274 +1：learnhub_concept_merge_candidates；#265 +1：learnhub_concept_confusable_candidates；#203 +1：learnhub_receipt_review_mode；#256 −1：learnhub_project_decompile_apply 随反编译 plan-only 退役）`)
  ...
  assert.equal(routeKeys.length, 130, `路由通道（panel+ops）应恰 130 条，实得 ${routeKeys.length}（#312 +1：POST /coach/draft/cancel；#274 +1：POST /concepts/merge-candidates；#209 +1：GET /compass；#215 +1：POST /smoke；#216 +1：POST /spike；#222 +1：POST /quality-review；#240 +3：POST /course/create、POST /endpoint/add、POST /endpoint/remove；#248 +1：POST /coach/stuck-report；#255 −1：GET /doctor 随 doctor 退役；#256 −6：GET /courses、POST /node/pin、/proposals/impact、/review、/seed/propose、PUT /day-cutoff 死路由退役）`)
...
test('门④ handler 覆盖：handlers.ts 的键集合 == 没有 bind 的路由通道（panel+ops）集合', () => {
  ...
  assert.equal(generic.length + handled.length, 130, '路由通道（panel+ops）总数应恰 130（#312 +1：POST /coach/draft/cancel；#268 +1：GET /concepts/footprint；#274 +1：POST /concepts/merge-candidates；（#209 +1：GET /compass；#215 +1：POST /smoke；#216 +1：POST /spike；#222 +1：POST /quality-review；#240 +3：POST /course/create、POST /endpoint/add、POST /endpoint/remove；#248 +1：POST /coach/stuck-report；#255 −1：GET /doctor；#256 −6：六条死路由退役）')
