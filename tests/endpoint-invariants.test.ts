...
import { validateAnchorBook, validateEndpointAnchor, readAnchors, foldCompletion } from '../src/engine/coach/seed.ts'
...
const SEALED_VAULT = {
  ...
  // 固定时钟（#175 纪律）：入门夹具的 R 随运行日衰减，gated 判据（前置 R ≥ 0.85）在真实
  // 时钟下 2026-09-19 起翻红——间隔 6 天 R≈0.846 跌破门槛，#199 软闸清单变空、断言退化。
  // 钉 2026-09-17（间隔 4 天 R≈0.879，过门槛的绿态），同一输入同一输出。
  clock: { nowMs: () => new Date(2026, 8, 17, 12, 0, 0).getTime() },
}
