...
test('G2 宿主模块可加载且装配面齐备', async () => {
  ...
  assert.equal(commands.COMMAND_LIST.length, 162, '注册表应为 162 条命令（#203 +1：receipt-review-mode；#215 +1：smoke；#216 +1：spike；#222 +1：quality-review；#240 +3：course-create、endpoint-add、endpoint-remove；#248 +1：stuck-report；#255 −1：doctor 并入 data-check）')
  assert.equal(Object.keys(commands.COMMANDS).length, 162, '按 id 键的装配表应为 162 个键（#159 +1、#203 +1、#215 +1、#216 +1、#222 +1、#240 +3、#248 +1、#255 −1）')
  assert.equal(commands.BY_TOOL.size, 112, 'agent 通道索引应为 112 条工具（#203 +1）')
  assert.equal(commands.BY_ROUTE.size, 133, 'panel 通道索引应为 133 条路由（#209 +1：GET /compass；#215 +1：POST /smoke；#216 +1：POST /spike；#222 +1：POST /quality-review；#240 +3：POST /course/create、POST /endpoint/add、POST /endpoint/remove；#248 +1：POST /coach/stuck-report；#255 −1：GET /doctor）')
