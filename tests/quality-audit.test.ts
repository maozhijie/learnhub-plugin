/**
 * 图质量面审计抽样（#224 / ADR-0070 §图质量面）：教练回合方向裁决质量的审计面声明、系统性
...
function stat(over: Partial<DimensionStat> = {}): DimensionStat {
  return {
    station: '教练执行', dimension: '补丁纪律', dimensionName: '补丁纪律（落图能过门）',
...
function low(over: Partial<LowScoreItem> = {}): LowScoreItem {
  return {
    ref: '教练执行/bad-桩.md', station: '教练执行', dimension: '补丁纪律', dimensionName: '补丁纪律（落图能过门）',
...
test('审计面清单：站名在语料词表、量规与维度在注册表——三张表漂移即红（审计不会静默跑空）', () => {
  ...
  assert.equal(AUDIT_AXES.length, 1, '种子轴退役后仅存教练回合方向裁决质量轴（#256）')
...
test('审计面声明：逐轴列出（含看点）；未抽到样本的轴照实说「本轮无样本」', () => {
  const lines = auditScopeLines({
    sampling: { corpusDir: 'C:/桩', stations: ['教练执行'], pool: 3, selected: 3, quota: { bad: 3, ok: 2 } },
    stats: [stat()],
    rubricIds: ['教练回合'],
  })
  const text = lines.join('\n')
  assert.ok(text.includes('审计面声明'))
  assert.ok(text.includes('教练回合方向裁决质量'), '轴在册')
  assert.ok(text.includes('已判 2 件次'), '在册轴的判读件次带出')
  assert.ok(text.includes('算子 + 理由 vs 图面'), '看点进声明')
})

test('系统性发现候选：低分率越预注册线才入候选；低分件引用与判据出处随候选带出（贴票用）', () => {
  const belowLine = stat({ dimension: '方向纪律', scored: 4, low: 1 })
  const aboveLine = stat({ scored: 2, low: 2 })
  const tooFew = stat({ dimension: '停摆与弧建议', scored: 1, low: 1 })
  const candidates = systemicCandidates([belowLine, aboveLine, tooFew], [low(), low({ ref: '教练执行/bad-桩2.md' })], CRITERIA_OF)
  assert.equal(candidates.length, 1, '只有越线维度入候选（样本不足/低分率不够都不入）')
  const c = candidates[0]!
  assert.equal(c.station, '教练执行')
  assert.equal(c.dimension, '补丁纪律')
  assert.equal(c.low, 2)
  assert.equal(c.scored, 2)
  assert.deepEqual(c.refs, ['教练执行/bad-桩.md', '教练执行/bad-桩2.md'], '低分件引用齐备')
