 ...
 *      #159/#209/#215/#216 各 +1 → 现值 129 条）。
 *   ② **行为快照**：473 条探针（每条路由 × 空参／全参／逐个缺参，外加分发纪律样本）
...
test('行为快照：473 条探针的状态码／响应体／引擎调用序列与基线逐字一致', async () => {
...
test('行为快照：状态码分布保持 200×226／404×6／500×241（探针口径；#209 +2×200/#215 +2×500/#216 +2×500）', () => {
  const dist = SNAPSHOT.reduce<Record<string, number>>((acc, s) => ({ ...acc, [s.status]: (acc[s.status] ?? 0) + 1 }), {})
  assert.deepEqual(dist, { 200: 226, 404: 6, 500: 241 })
})
