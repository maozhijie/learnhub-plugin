...
test('行为快照：485 条探针的状态码／响应体／引擎调用序列与基线逐字一致', async () => {
...
test('行为快照：状态码分布保持 200×229／404×6／500×250（探针口径；#209 +2×200/#215 +2×500/#216 +2×500/#222 +2×500；#240 −1×200/−1×500（seed/propose 删 goal/mode 探针）+4×200/+8×500（建课改模式三路由））', () => {
  const dist = SNAPSHOT.reduce<Record<string, number>>((acc, s) => ({ ...acc, [s.status]: (acc[s.status] ?? 0) + 1 }), {})
  assert.deepEqual(dist, { 200: 229, 404: 6, 500: 250 })
})
