 ...
 *      #159/#209/#215/#216/#222 各 +1 → 现值 130 条）。
...
test('行为快照：状态码分布保持 200×226／404×6／500×243（探针口径；#209 +2×200/#215 +2×500/#216 +2×500/#222 +2×500）', () => {
  const dist = SNAPSHOT.reduce<Record<string, number>>((acc, s) => ({ ...acc, [s.status]: (acc[s.status] ?? 0) + 1 }), {})
  assert.deepEqual(dist, { 200: 226, 404: 6, 500: 243 })
})
