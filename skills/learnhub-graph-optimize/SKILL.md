...
description: 优化 learnhub 既有课程知识图谱（改名/调前置/增删节点），走 propose-edit 门禁后 apply 入库并自动联动笔记与引用。用于用户要求调整课程结构时。
---
...
reason: 为什么改（一句话）
ops:
  - { op: add_node, name: 新节点名, pre: [前置], note: 说明 }
...
- 删除节点前确认它没有学习笔记沉淀；有则优先 set_note 标记。
