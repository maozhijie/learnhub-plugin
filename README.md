...
2. **逐节正文**：每节一次模型调用（「课程节生成」提示词 + 节任务 + 前节已生成正文保连贯 + 上下文包）→ 质检门 apply 单节落盘，门禁未过把完整清单回灌模型自动修复一轮（仅一轮，修复轮仍失败才报错）；已有 ready 节（断点续跑）沿用既有清单，只补未生成节。大纲与逐节正文走快速思考档（`fastEffort` 可配，默认 off——文字量小、模板约束强，无需推理；出题/批改/答疑保留默认思考）。
