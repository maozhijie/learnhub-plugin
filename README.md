...
| 日志 | `state/学习日志.md`、`state/logs/<日期>.log` | 学习流水 + 调试日志（引擎关键路径可观测，按天一个文件、保留 30 天） |
...
- 图谱质量机制：健康分 5 项（动作句命名/est 覆盖/前置完备/收敛度/结构卫生）进审计基线表；审计 R13 认知跨步（相邻 pre 边 |Δdifficulty|≥2 → warn，原 R11 口径并入）与 R12 认知-时长失配（info）；生长批质量由受理门把守（schema/结构检查/概念对表/终点锚保护/巩固门/插入批调速闸），受理门拒收把门错误回灌教练重裁一轮（仍拒才失败，失败通知/生成队列带「重试」；ADR-0050），批次节奏由教练回合提示词约束（每批 ≤8 操作、单算子方向，apply 后看清全图再裁下一批）；数量限定按图规模伸缩（健康分别名命中按节点数归一；R1 浅叶阈值随最大深度相对化；审计条目超 15 附溢出行）；正文落盘时 enc_candidates 引用非祖先节点 → 返回值附 hints 提醒补边（内容反哺图）
...
`/learnhub/` 五区页签（今日 / 课程 / 洞察 / 项目 / 无界实践区），按使用节奏组织，两级路由（`#/today`、`#/courses`、`#/course/<课程>/<分栏>`、`#/queue`、`#/proposals`、`#/insight`、`#/projects`、`#/practice`——刷新保持当前视图、前进后退在历史间穿梭、深链直达；旧键深链回落默认页签）；明暗双主题由自建 token 层驱动（arco-theme 跟随系统，可手动切换）：

- **今日**（日循环，默认区）：XP/streak/每日目标 + 复习横幅（待复习数 + 开始复习）+ **「接下来」推荐流**（逾期/复习/就绪/新学大卡片，复习项来自题库聚合 due，生成中的节点带逐节进度，点开直接进入节点学习视图）+ 课程供给卡（在酿 n / 停摆恢复 / 失败重试，与生成队列同源）+ 待审闸门计数（点击直达提案收件箱）+「我的资产」菜单（我的卡 / 笔记源抽屉）。课程卡列表不进今日（住课程区），习惯自报不主动拉出（无推送纪律）
- **节点学习视图（LessonView）**：今日页二级全屏视图，最大最丰富的学习容器——状态引导单按钮（无正文→「生成正文（自动出题）」；有正文无题→「AI 出题」；有题→直接练习）+ 顶部节进度 stepper（MathAcademy 式细条分段，节序列驱动：内容节 = 读 + 本节练习并入一个步骤、`练习` 节 = 一等练习轮、`交互` 节 = 交互模拟轮；已过关蓝条可点回跳，当前高亮，节名在 tooltip）+ 正文分节 Markdown 渲染（GFM 表格 / mermaid 图 / svg 示意图 / plot 函数图 / chart 数据图表 / Obsidian 图片嵌入 / interactive 交互件；每节标题旁「重写」让 AI 单节重生成过门落盘）+ 练习区（题目 FSRS 到期排序 + 九种题型 QuestionCard）+ 掌握度（题目作答汇总）+「完成学习」与「跳过」按钮（完成按钮在练习区下方，做完题再确认）+ 生成中阶段/进度/耗时/取消（`/generate/status` 附 `contentVersion`，正文版本变化才整页刷新）+「在图中查看」低频跳转 + 右下角「问 AI 老师」抽屉（节点范围即时答疑）+「与 AI 讨论本课」（把节点上下文注入新建 dsh 会话，深度讨论/修订）
- **课程**（课程机器，三入口）：**我的课程**（课程卡网格：进度/六态计数/逐课复习/重新生成/删除；空库时是建课入口——教练台种子表单，全面板唯一主动建课入口）→ **单课工作台**（`#/course/<课程>/<分栏>`：首屏罗盘+图；教练台分栏＝建课/生长一步/就绪深度/恒温器/回填/复诊；生长与队列、提案、题库三个本课切片分栏）；**生成队列**（全局面：正文生成任务注册表——运行中/取消/终态，落盘 `state/生成任务.json`，刷新可恢复、服务端为事实源，失败与部分完成保留 24h + 完整错误与重试 +「与 AI 讨论」直通 dsh 会话，进度列 mini 进度条 + `done/total · 当前节`，风格 Select 列可用变体）；**提案收件箱**（全局面：人审唯一处——种子/编辑/富化/项目草案/反编译双提案/N-of-1 实验开跑，应用前可看影响预览，应用后按类型给「查看结果」落点）
- **洞察**（周期循环的观测与自我实验）：周复盘 Weekly Kata（五问，现状引擎自动填，「下一实验」一键转 N-of-1 提案或执行意图）+ 记忆健康（负载预报/状态分布/真实保留率/遗忘曲线 + FSRS 参数优化器 + JOL 抽样率与过信轻提示开关）+ 自评校准画像 + 沙盘（计划推演，分位带，非承诺）+ N-of-1 实验（模板发起 → 收件箱确认 → 报告定稿）+ 睡眠耦合建议开关 + Anki 通道（导出/回写）+ XP 时间账本（今日 XP / streak / 每日目标）+ 课程六态总览与 ETA + 运行环境（当前模型与思考档只读）
- **项目**（做事的人）：执行事件/里程碑/渐退档/掌握交叉原样（P 区，Project 是 Course 姊妹实体）
- **无界实践区**：技能条目与执行事件（乐器/运动等持续技能的调度 lane）；能力指南退役为壳级帮助抽屉，顶栏留通知铃铛位
