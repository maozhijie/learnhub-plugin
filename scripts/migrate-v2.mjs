 ...
 * 先把它移出 学习中心 另存，再跑本脚本。
 *
 * 与 migrate-v1.mjs（已按裁决退役为存根）**刻意各自独立、不抽共享库**：断裂脚本是
 * 一次性冻结产物，跑完即退役（本文件同样在那之后留作存根）；为两次断裂抽共同体会
 * 在第一次断裂退役后变成没人维护的第二份代码，而它要处理的正是「磁盘形状不可信」
 * 的局面——共享抽象的收益（省 100 行）远低于读错一次旧库的代价。
 */
