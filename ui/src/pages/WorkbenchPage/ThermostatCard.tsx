...
export default function ThermostatCard() {
  ...
  return (
    <Card size='small' title='挑战点恒温器 · 跨区观测（只读）' className='lh-card lh-mt-10'>
      {thermo ? (
        <div className='lh-grid lh-gap-10'>
          <div className='lh-flex lh-gap-24 lh-wrap'>
            <div>
              <Text type='secondary' className='lh-t-11'>课程区 · 真实保留率（{thermo.course_region.retention.real} 次到期复习）</Text>
              <div><Text bold>{thermo.course_region.retention_band.label}</Text></div>
            </div>
            <div>
              <Text type='secondary' className='lh-t-11'>课程区 · 难度带长期选择（{thermo.course_region.band_choices.sessions} 次会话）</Text>
              <Text className='lh-t-12'>
            ...
            <div>
              <Text type='secondary' className='lh-t-11'>无界区 · 执行事件评级</Text>
              <div><Text className='lh-t-12'>{thermo.unbounded_region.execution_ratings.count
                ? `${thermo.unbounded_region.execution_ratings.count} 次`
                : '暂无数据（随执行事件通道上线）'}</Text></div>
            </div>
            <div>
              <Text type='secondary' className='lh-t-11'>项目区</Text>
              <div><Text className='lh-t-12'>{thermo.project_region.projects.length
          ...
          {thermo.suggestions.length ? (
            ...
            <Text type='secondary' className='lh-t-12'>暂无建议（低数据静默——观测足够后才会出现至多三条，逐条确认才生效；恒温器不是自动控制器）。</Text>
          )}
        </div>
      ) : (
        <Text type='secondary' className='lh-t-12'>加载中…</Text>
      )}
    </Card>
  )
}
