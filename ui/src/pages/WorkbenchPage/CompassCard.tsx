 ...
 * 的面板读视图——剩余路线（罗盘站独占写权；生长批入队前弧未画时引擎自动代拉初画，
 * 重估重画走显式下发）、学习者批注区（软输入：提议非指令）、每周挂载的沙盘 ETA
 * （措辞「模型推演，非承诺」锁死）。数据走
...
export default function CompassCard({ course }: { course: string }) {
  ...
  return (
    <Card size='small' title='罗盘' className='lh-card'
      ...
      {doc.missing ? (
        <Empty description='罗盘还没有初画：下一次生长批入队时会自动代拉初画，也可在教练台显式下发。' />
      ) : (
        <Space direction='vertical' size={10} className='lh-full'>
          <div>
            ...
            {doc.route
              ? <MdView md={doc.route} />
              : <Text type='secondary'>待初画：下一次生长批入队时会自动代拉初画；也可在教练台「罗盘初画/重画」显式下发。</Text>}
          </div>
          <div>
