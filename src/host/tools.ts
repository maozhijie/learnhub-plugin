...
import { resolveEngineEntry, run } from './runtime.ts'
...
export const AGENT_GUIDE: Array<{ tool: string; page: string; text: string; prompt?: string }> = [
  { tool: 'learnhub_pin_today', page: 'today', text: '「今天学它」：把节点置顶为今日推荐榜首（可挂执行意图），只影响今天、次日自动失效。',
    prompt: '用 learnhub_pin_today 把「<节点>」设为今天的学习目标' },
  { tool: 'learnhub_unpin', page: 'today', text: '取消今天的「今天学它」置顶。', prompt: '取消「<节点>」的今日置顶' },
  { tool: 'learnhub_goal_intention', page: 'today', text: '在今日 pin 上写/清「在【线索】之后就【行动】」的执行意图（随 pin 当日过期）。',
    prompt: '给今天「<节点>」的 pin 挂一个执行意图：晚饭后就在书桌前学完它' },
  { tool: 'learnhub_note_source_exclude', page: 'today', text: '把个人笔记文件/目录加入排除清单（未来不再被自动注册为复习源）。',
    prompt: '把「<笔记路径>」加入笔记源排除清单' },
  { tool: 'learnhub_note_source_unexclude', page: 'today', text: '从笔记源排除清单移除（恢复可注册资格）。',
  ...
  { tool: 'learnhub_question_get', page: 'courses', text: '读单题全文（含答案与解析）——改题/审题前先看原题。',
    prompt: '把「<节点>」题库里 q1 的完整题目读给我看' },
  { tool: 'learnhub_bank_cleanup', page: 'courses', text: '一键清理题库：跳过节点的全部未归档题 + 已完成节点的休眠题（从未调度），预览确认后归档（可逆，不删除）。',
    prompt: '预览一下题库清理会归档哪些题，我确认后再执行' },
  { tool: 'learnhub_content_check', page: 'courses', text: '只跑正文质检门不落盘——在 vault 手改笔记后自检违规。',
    prompt: '对「<节点>」跑一次正文质检' },
  ...
  { tool: 'learnhub_receipt_submit', page: 'practice', text: '提交外部练习回执（描述/图片/导出皆可；评审模式按当日生效档 = 实验臂或默认 AI，零 XP、不推调度；自评臂带 self_score）。',
    prompt: '提交一份回执：<练习内容描述>' },
  { tool: 'learnhub_receipt_list', page: 'practice', text: '查看已提交的回执清单。', prompt: '列出我提交过的回执' },
  { tool: 'learnhub_receipt_review_mode', page: 'practice', text: '读/设回执评审默认档（ai/self；回执评审实验在跑时当日臂覆盖默认）。',
    prompt: '把回执评审默认档改成自评' },
...
export function registerTools(ctx: Context, rt: HostRuntime): void {
  ...
  for (const c of COMMAND_LIST) {
    ...
    const execute = bind !== undefined && engine !== undefined
      ? (args: Record<string, unknown>) => run(rt, tool, async () => JSON.stringify(await resolveEngineEntry(rt, engine)(...boundArgs(args, bind) as never[])))
      : handlers[tool]
