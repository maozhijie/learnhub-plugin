...
export const 图谱域 = {
  ...
  'concept-merge': command({
    id: "concept-merge",
    summary: "File a CONCEPT-MERGE PROPOSAL — this does NOT merge anything by itself (概念登记表 #141, #265 / ADR-0084: merging is the one irreversible action in this system, so it goes through the same proposal-then-human-confirmation path as graph changes). Nothing is written while the proposal is pending: the learner must confirm it in the panel (提案列表 → 应用, kind=concept_merge) before the absorbed entry disappears and ALL of its names (canonical + aliases) become aliases of the surviving entry, so every historical address keeps resolving — entries are never deleted, only merged. The proposal surface states this irreversibility explicitly (只并入、不拆分); a merge cannot be undone by splitting. Use when the same concept was minted twice under different names (same meaning, confirmed by the learner); deepening a concept to a higher tier REUSES the same entry and is NOT a merge. The receipt carries the resulting name set plus non-blocking warnings (近似名候选 / 量级告警带), and the proposal id; the registry file is 课程根/概念登记表.yaml.",
    args: {
  ...
  'concept-confusable-candidates': command({
    ...
    summary: "Derive confusable-PAIR CANDIDATES from question co-occurrence and file one PENDING proposal per candidate (概念登记表 #265 / ADR-0084 ③): sources are the concepts invoked by different questions of the same node, and the node's misconception concepts against its questions' invokes. Nothing is registered automatically — the learner reviews the candidates one at a time in the panel (提案列表 → 应用, kind=confusable_pair), and only then does a pair enter the registry (one direction only: a→b — a one-way declaration is LEGAL but is a to-be-reviewed state, not a finished one). Evidence lines ship inside each proposal so the human can judge whether the two concepts really are confusable. Non-negotiable filters: names must be registered and ACTIVE (deprecated entries leave the candidate surface, ADR-0084 ②), already-declared pairs are never re-nominated, and one scan files at most `max` proposals (people are the bottleneck). Re-runnable: pending duplicates are not stacked.",
    args: {
  ...
  'graph-apply': command({
    id: "graph-apply",
    summary: "Decide a pending graph proposal: apply (audit-gated, writes data/*.yaml with rename linkage + journal + snapshot; kind=enrich re-checks the sha256 content fingerprints and refuses stale proposals; kind=seed lands the drafted structure on an ALREADY-registered course — appends the drafted endpoint to state/终点锚.json only when the name is free, existing anchors are never touched; seed graphs get the shape-warning & health-threshold exemption) or reject (kept on record). Seed proposals (kind=seed) are the structure-drafting channel for registered courses — they never create courses, and endpoints are hand-added/removed by the learner via the panel (POST /endpoint/add, POST /endpoint/remove); they always wait for the one human review. In edit batches the agent applies directly after gates pass; revision changes wait for human review first (ADR-0003). The apply result carries findings: audit warns plus a health-score hint when below the quality baseline (suppressed while the graph is still just the seed) — address them in the next batch.",
    args: {
  ...
  'graph-proposals': command({
    id: "graph-proposals",
    summary: "List proposals by status — use status=pending to see what awaits human review in the panel, with the proposal id, course, reason, and op summary. Concept-layer actions (#265: concept_merge = an irreversible entry merge, confusable_pair = a derived confusable-pair candidate) also queue here: the learner decides them in the panel one at a time. After the user decides in the panel, apply graph proposals with learnhub_graph_apply using that id (concept-layer ones are panel-only — 确认门是人的动作).",
    args: {
  ...
  'graph-propose': command({
    id: "graph-propose",
    summary: "Submit a graph proposal. Schema quick reference — write YAML strictly to this, wrong key names are rejected. Anchors are human-authored (ADR-0076): add/remove endpoints are explicit panel actions, and the endpoint-anchored node is still guarded — del_node/rename via kind=edit are rejected. kind=edit (per batch) top-level keys: course; reason?; concepts? ([{canonical, aliases?, definition?}] — concept-registry minting block, #141: names land in 课程根/概念登记表.yaml with the SAME apply transaction, nothing written while the proposal is pending/rejected); ops[] — add_node defines a new node via key `name` (unified with graph YAML in schema v2; the old `node` key is rejected): add_node{name, pre, est? (minutes, positive), bloom? (记忆/理解/应用/分析/评价/创造), difficulty? (1-5), type?: practice, note?, enc?, teaches? ({concept: 知道|会用|能教}, 1-8), assumes? ({concept: tier}, 3-10 when present), misconceptions? ([{concept, model}], ≤3 per concept course-wide)}; every other op targets an existing node via key `node`: set_pre{node, pre} replaces the whole pre set (pre is required, [] to clear); set_enc{node, enc} replaces the whole enc list ([skill] or [{node, w, note}]; enc is required, [] to clear); del_node{node}; rename{node, new}; set_note{node, note}. Generation-time discipline the gates cannot check (ADR-0040): each add_node is ONE independent learning act — self-question whether a zero-basis learner could pick it up from its pre within 30 minutes, else split or add a prerequisite first; most names are action sentences (解/求/推导…, no「理解导数」-style level-unclear near-duplicates, no blanket compounds); pre is the COMPLETE direct-prerequisite set (no transitive padding); give every new pre edge a necessity verdict (delete-the-edge test: no concrete failure point = redundant, drop it). Edge-light rule: graph YAML carries ZERO edge metadata — candidate edges stay in the proposal, insertion origin derives from the proposal journal, probation lives in state/边实验.jsonl (fields like origin/status/probation are rejected). Growth-batch note region (#145/#146, ADR-0076): note{operator: 前进|插入|巩固|旁支|换向, reason, target_endpoints?, disagreement?, recheck?} marks the batch as a coach-round verdict; a growth batch (前进/换向) with add_node ops MUST declare target_endpoints — a non-empty list of registered endpoints this batch grows toward (one new node may enter several endpoints' pre: junction nodes are legal), and the gate checks EVERY declared endpoint carries set_pre{node: <endpoint>} covering all in-batch new-frontier nodes (add_node with an endpoint as pre is rejected — never grow past the target; 换向 no longer edits anchors — anchoring is human-authored); an insertion batch (operator=插入 with add_node ops) MUST preregister its recheck — note.recheck{metric: 前进恢复|卡点集中度降幅|保留率恢复 (exactly one, same source as the symptom), days? (learning days, default 10, clamped to [5,20] with a warn)} — the engine auto-adjudicates at expiry (proven, or auto-prune via del_node + coarse-edge restore, zero human review); recheck on non-insertion batches is rejected, and insertion batches are gate-throttled when the recheck pass rate bottoms out or the insertion/side-branch share exceeds its cap. Batch `pre` may only reference existing nodes or nodes created earlier in the same batch. Concept-reference gate (#141): every concept named in teaches/assumes/misconceptions must be registered in the course concept registry (canonical or alias, exact match) OR minted in the same proposal's concepts block — unregistered names are rejected with the missing list; minting a name that already exists is rejected too (reference the entry, or file a merge proposal with learnhub_concept_merge and let the human confirm it — merging is irreversible: 只并入、不拆分). Minting a name that is not identical but is character-level NEAR to a registered name comes back in warns as a candidate list (non-blocking, #264): prefer referencing the existing entry, or state the difference in the batch reason; entries also carry magnitude warning bands (alias count / confusable-pair count) in the receipt. Prior feed (#142): vault-link candidates with w≥0.7 mapped to the proposal's nodes that the structure does NOT explicitly answer (no pre/enc edge between the pair) come back in warns (non-blocking) — answer them with real edges, or let a vault rescan drop them; zero priors is a legal normal path. Keep pre-edge cognitive jumps (difficulty gap >= 2 or depth span >= 3) off the graph or expect R13 jump-candidate warnings. Schema + structure gates reject bad YAML with actionable errors (including dangling enc edges and misconception cap breaches). In growth batches apply edit proposals immediately after gates pass (anchor-review model, ADR-0003).",
    args: {
