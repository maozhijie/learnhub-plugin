...
export const 学习域 = {
  ...
  'coach-growth': command({
    id: "coach-growth",
    summary: "Run one coach round for a course and apply the resulting growth batch: enqueues ONE growth job on the generation queue (phase=growth) and waits for its terminal message. The round runs a two-station relay — 思路官 produces the direction plan, 执行官 lands the structure through the draft circuit — and the batch is a kind=edit proposal that applies automatically once the gates pass (no per-batch human review). Structure only: no content is enqueued, so the receipt names the created nodes and you generate their bodies separately. Rejected when the course has zero endpoints, when the queue is paused (resume from the generate page), or while a batch is already running/cancelling. Budget: one deep tool loop plus one fast plan call — do not call it repeatedly to 'hurry' a course along.",
    args: {
