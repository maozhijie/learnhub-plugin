...
export const 学习者产出域 = {
  ...
  'error-card-archive': command({
    ...
    summary: "Archive or restore one 错误对比卡 (management): archived cards leave the review queue but keep their history in the card file; a question with only an archived card becomes minable again on the next generate run. Error-deck internal action: zero canonical writes.",
    args: {
      course: { type: "string", description: "Course name", required: true },
      node: { type: "string", description: "Node the card belongs to", required: true },
      card: { type: "string", description: "Card id, e.g. \"c1\"", required: true },
  ...
  'explain-feedback': command({
    id: "explain-feedback",
    summary: "Close a「讲给我听」session with located feedback: sends the full transcript to the grading model against the node's content points and returns verdict (对/部分对/错) + deviation tags (含糊/跳跃/说错) + a \"how to fill the gap\" advice + the full markdown feedback for the learner. The verdict is archived ONLY in the learner-output archive — zero XP, zero canonical writes; fails loud with zero side effects if the model output is unparseable.",
    args: {
      course: { type: "string", description: "Course name", required: true },
  ...
  'habit-create': command({
    id: "habit-create",
    summary: "Create a habit: an execution intention (cue + action, format locked to \"stable time/place cue → ONE concrete action\") + an automation curve + a forgiving streak. Habits have NO FSRS semantics, no mastery, NO due dates — the scheduler is context and calendar, the engine never reminds. Repetitions are self-reported (no gate, no anti-cheat — self-measurement is not an exam).",
    args: {
  ...
  'habit-repeat': command({
    id: "habit-repeat",
    summary: "Self-report one repetition of a habit (the ONLY counting source; unlimited, no gate). Optionally carry an automation self-rating 1-5 (event-level, not required every time). Zero XP, zero scheduling writes — habit repeats never enter the execution-event lane or any ledger.",
    args: {
      habit: { type: "string", description: "Habit id", required: true },
  ...
  'kata-convert-experiment': command({
    ...
    summary: "One-click exit from the Kata's「下一实验」to an N-of-1 experiment proposal: files the SAME proposal-confirm flow as learnhub_experiment_propose (nothing starts by itself) and stamps the proposal number into the Kata record. Requires the week's record to exist (learnhub_kata_open first).",
    args: {
  ...
  'learner-card-archive': command({
    ...
    summary: "Archive or restore one「我的卡」self-note card (management). Archived cards leave the learner queue but keep their history in the card file. Card-domain internal action: zero canonical writes.",
    args: {
      course: { type: "string", description: "Course name", required: true },
  ...
  'learner-forget': command({
    id: "learner-forget",
    summary: "Declare「忘记」on a「我的卡」self-note card — the learner could not restate it, so the card is pushed with rating 1 (again tomorrow). One push per card per day; zero XP (a 0-XP unbound journal row keeps the streak ledger honest), no mastery or node-scheduling writes.",
    args: {
      course: { type: "string", description: "Course name", required: true },
      node: { type: "string", description: "Node the card belongs to (source_node)", required: true },
      card: { type: "string", description: "Card id, e.g. \"c1\"", required: true }
  ...
  'learner-queue': command({
    id: "learner-queue",
    summary: "List ALL「我的卡」cards for inventory/management: due cards first (due ascending), never-scheduled cards after. Each card carries prompt (front: what to restate) and content (back: the learner's own wording), source_node/source_section anchors, and attempts. Review happens in the merged cross-course review queue or directly via learnhub_learner_rate (Hard/Good/Easy 2/3/4) / learnhub_learner_forget — one push per card per day. Rating earns unbound XP: counted in totals/daily goal/streak only, never in per-course/per-node ledgers, never in mastery.",
    args: {
      course: { type: "string", description: "Course name; omit for all enabled courses", read: "query" }
    },
  ...
  'skill-maintenance': command({
    ...
    summary: "Set a skill's maintenance beat cap (days 7-365, or null to disable): the lane comes due at most this many days after the last execution, so interval growth can never drown the skill (disused motor skills decay hard — low-frequency contact itself has value). Pure entity property: the existing FSRS state is untouched.",
    args: {
      skill: { type: "string", description: "Skill id", required: true },
  ...
  'understanding-add': command({
    ...
    summary: "Add the learner's「我的理解」as a section-anchored self-note (「加我的理解」entry): the learner writes ONE explanation/example/mnemonic in their OWN words for a section they just learned; the model compares it against that section's taught points and returns verdict (对/部分对/错) + located deviations (含糊/跳跃/说错) + a \"how to fill the gap\" advice + the full markdown feedback. The verdict is archived ONLY in the learner-output archive and the wording becomes a LearnerCard in the「我的卡」domain (reviews ride the merged review queue and earn unbound XP — totals only, never per-course ledgers) — creating is zero XP, zero mastery/node-scheduling writes. Unparseable model feedback fails loud with zero side effects (nothing is archived, no card is created). kind: recall_cue 提示重述 (default) / cloze_rewrite 挖空重述 (content must contain a non-empty {{…}} cloze) / self_explain 自注讲解.",
    args: {
      course: { type: "string", description: "Course name", required: true },
      node: { type: "string", description: "Node name", required: true },
      content: {
