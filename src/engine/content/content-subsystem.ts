...
import { deprecatedNames } from '../concepts/concepts.ts'
...
import { shuffledWith } from '../infra/shuffle.ts'
import type { Sessions } from '../sched/sessions.ts'
import type { LearnerCards } from '../learner/learner-cards.ts'
...
import { bandOffset, combinedDifficulty, sessionOrder, startBand } from '../sched/adaptive.ts'
import { advancePending, advanceStrict, alreadyAdvanced } from '../sched/advance.ts'
import { effectiveStage } from '../graph/audit.ts'
import { calibrationHintText, overconfidenceOf } from '../sched/calibration.ts'
import type { ComplexityTier } from './complexity.ts'
import { nodeTierOf, sectionTierLabel } from './complexity.ts'
import { OPEN_QUESTION_GRADING_SYSTEM, PASS_SCORE, REFLECTION_GRADING_SYSTEM, answerDiff, applyPracticeEvidence, clamp01, evaluateAllo, parseOpenGrading, parseReflectionGrading, revealAnswer } from '../infra/grading.ts'
import { safeFilename, groupView, type GroupAxis } from '../graph/graph.ts'
import { jolDeviatedKeys, pickJolTargets } from '../sched/jol.ts'
import type { LearnerCardDoc } from '../learner/learner-cards.ts'
import { interleaveBySource } from '../coach/nof1.ts'
import { NOTE_SOURCE_COURSE } from '../vault/note-source.ts'
import { asFm, loadNote, saveNote } from '../vault/notes.ts'
import { endpointNames, readAnchors } from '../coach/seed.ts'
import { CALIBRATION_BOOST_SAMPLE_RATE, FSRS_DIFFICULTY_MID, XP_GUESS_SECONDS } from '../infra/params.ts'
import { assertNoBrokenNotes } from '../sched/sessions.ts'
import { masteryOfFm, previewDue, retrievabilityBlock } from '../sched/srs.ts'
import { sourceKeyOf } from '../types.ts'
import type { FsrsBlock, ReviewRec, SectionManifest } from '../types.ts'
import { priorSection, queryEntriesFor, runPriorSearch } from '../vault/vault-prior.ts'
...
import { xpForAnswer } from '../sched/xp.ts'
