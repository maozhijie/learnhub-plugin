...
export const PROMPT_CHANGELOG: Readonly<Record<string, readonly PromptBump[]>> = {
  ...
  罗盘初画: [{
    ...
    // 罗盘初画提示词人工修订（散文冻结令解除后用户亲手落笔，AI 只补登记与过门）。
    version: 5, date: '2026-09-18', changeType: '人设与完整性改写（用户手改）：领域大师→统筹规划者 + 能力面完整覆盖硬要求 + 深度档改必标 + 删（候选）标注与批注条款',
    expectedDelta: '人设句改为「统筹规划者」；新增「划分一定要专业，完整覆盖所有能力面，宁多勿少」硬要求；硬约束 3 增「更建议同一能力面出现在多个终点下」；深度档从可选改为在条目里必标（删「不标 = 未声明，不推定」）；删「未落图的阶段一律标（候选）」与「学习者批注」软输入条款（重画版改为正文句保留）。预期增量：弧更完整、跨终点复用面更受鼓励、阶段条目全部带深度档（周检讨「需求 vs 现状」读数的判域变宽）；（候选）标注退场后候选面判读改为「未落图即候选」。过门记录：check ✓（同提交登记）；replay ✓（0 件回归）；compare 未做真模型对照',
  }, {
    // 罗盘语义纠正（用户裁决 + 授权落笔）：罗盘是「终点的构成分解」，不是「从当前出发的
    // 路线」。三处改动同批：① 开头句「从学习者当前的水平到那个程度状态，中间要长出哪些他
    // 能做的事」改「把差距分解为他要具备的构成项」，并显式声明「条目之间不表达先后」——
    // 旧句是「罗盘第一条 = 下一站」误读的规则侧根因（实测：教练每会话与它搏斗一次）；
    // ② 双视角自检（领域结构侧 × 学习者实做侧，两边对照查整块遗漏——数学课几何缺席即此
    // 类盲区）；③ 「宁多勿少」保留并落成可执行自检动作。域内自洽（不提图/教练/下游机制，
    // 两提示词域互不可见，一致性靠接口文本与两边教导语义相容）。
    version: 6, date: '2026-09-20', changeType: '罗盘语义纠正：路线→终点构成分解（差距分解 + 条目不分先后）+ 双视角遗漏自检',
    expectedDelta: '开头句改为「把从学习者当前的水平到那个程度状态的差距，分解为学习者要具备的构成项……条目是并列的构成项，不表达先后」；覆盖要求句尾增「枚举完成后换一个视角自查一遍遗漏：既从领域知识结构出发枚举，也从『学习者在实际中要做的事』出发枚举，两边对照——只出现在一边的整块能力就是疑似遗漏，补进清单」。预期增量：弧仍是终点承诺的能力面展开，但不再被读成有序行军路线（教练侧顺序权回收）；双视角自检逼出「只在实际实做侧可见」的整块能力（实测第二轮：新课程罗盘补出常微分方程初阶/复数/思想史三块）；风险面：自检动作可能被物化为清单条目（实测第三轮零思考档下出现「自查补漏（…）」假条目），教练侧教导已配套（条目对照不写入 note），罗盘侧留观察。过门记录：check ✓（同提交登记）；replay ✓（0 件回归；罗盘站不在回放面，同 v1/v5 先例）；compare 未做真模型对照',
  }],
  罗盘重画: [{
    ...
    // 罗盘重画提示词人工修订（与初画 v5 同批：用户亲手落笔，AI 只补登记与过门）。
    version: 2, date: '2026-09-18', changeType: '人设与完整性改写（用户手改，与初画 v5 同批）：统筹规划者 + 能力面完整覆盖 + 深度档必标 + 批注条款上移正文 + 删（候选）标注',
    expectedDelta: '人设句改为「统筹规划者」并拆段；「学习者批注是软输入」从硬约束 6 上移为正文句；新增「划分一定要专业，完整覆盖所有能力面，宁多勿少」；硬约束 3 增「更建议同一能力面出现在多个终点下」；深度档必标；删「未落图阶段一律标（候选）」。预期增量与初画 v5 同向：弧更完整、深度档全量在场、候选面判读改为「未落图即候选」。过门记录：check ✓（同提交登记）；replay ✓（0 件回归）；compare 未做真模型对照',
  }, {
    // 与初画 v6 同批（用户裁决：罗盘是终点构成分解，不是路线；两族教导严格同文）：
    // 开头句改差距分解 + 条目不分先后；覆盖要求句尾增双视角遗漏自检。重画独有的边界句
    // （常驻地形感/改弧不改写已学事实）与批注软输入句原样保留。
    version: 3, date: '2026-09-20', changeType: '罗盘语义纠正（与初画 v6 同批）：路线→终点构成分解 + 双视角遗漏自检',
    expectedDelta: '与初画 v6 同向：开头句改「差距分解 + 条目是并列的构成项，不表达先后」；覆盖句尾增双视角对照自检。重画语境差异保留（重估不例行、批注软输入、改弧不回滚已学事实）。预期增量：重画产出的弧不再被读成有序路线；重画时对整块遗漏的再发现能力与初画同强。过门记录：check ✓（同提交登记）；replay ✓（0 件回归；罗盘站不在回放面，同 v1/v2 先例）；compare 未做真模型对照',
  }],
  教练执行: [{
    ...
    // 教练执行提示词人工修订（用户亲手落笔；AI 只补登记与过门）。与 ADR-0105 呼应：
    // 依赖契约的充分性条款在此升为模型面散文，门级执法随 ADR-0105 实现票另刀。
    version: 7, date: '2026-09-18', changeType: '教练人设与依赖契约改写（用户手改）：教育专家人设 + 罗盘指导口径改写 + 网络完整性与冗余措辞收紧 + 新增【空图额外建议】【依赖契约】两节',
    expectedDelta: '人设句改为「具有教育领域专业知识的专家」，生长叙事改为「从现状朝学习者的 n 个终点发展」；罗盘指导条改「罗盘是对终点的专业说明，推进时尽量并行照顾罗盘中的路径」（替换「方向由终点携带，由罗盘指导」）；网络完整条改「关系不能冗余」并上移到构建准则；新增【对于只有终点的空图的额外建议】（多路线并行穿插）；新增【依赖契约】四条：充分性（SUFFICIENCY，pre 完整且前置足够，否则严重错误）、收敛（真实知识是 DAG 不是树）、非入门单元 pre 不得为空、连通性（交叉链接）。预期增量：模型对 pre 集合的完整性与网络连通性自我要求升格（门级执法随 ADR-0105 概念充分性门另刀落地）。过门记录：check ✓（同提交登记）；replay ✓（0 件回归；教练执行 tool-calls 不在回放面，如实登记）；compare 未做真模型对照（同先例）',
  }, {
    // #337 落笔轮：充分性条款手改为用户亲笔（冻结期人工修订）；node_card 难度三常量与两条
    // 工具描述为 #337 评论授权面（用户明示「工具类提示词改动授权 Agent 落笔」）。
    version: 8, date: '2026-09-18', changeType: '依赖契约充分性升格硬校验（#337 用户手改）+ node_card 难度入卡（#337①/#338①，工具面授权落笔）',
    expectedDelta: '【依赖契约】删万能自我豁免句「当然如果确信直接前置节点已经隐含了依赖内容，则不必非要验证整个关系树」，替换为「概念反查（受理门硬校验，必做）」条（assumes 概念须在前置闭包内被 teaches 且档位不低于假定，否则整批拒收；缺口两条路：新增台阶或 set_pre 回连；本课首引须 note 说明）。同波工具面：node_card 视图新增难度行（未声明给「未标注」占位不省略行）、pre 条目内联前置难度；node_card 工具描述（活图/草稿两口径）字段清单补「难度（本节点与各前置）」。预期增量：模型不再照豁免句生成注定被概念充分性门拒收的批；难度步进门（#335）判据在卡上可算（本节点 vs 直接前置最大难度）。过门记录：check ✓（同提交登记）；replay ✓（0 件回归；教练执行 tool-calls 不在回放面，如实登记）；compare 未做真模型对照（同先例）',
  }, {
    // #338（同版本号第二条登记，模板文本未动）：教练包终点状态行取值变化——包头与
    ...
    // #341 A 项：铸名纪律散文（冻结期用户亲手落笔；AI 只补登记与过门）。与 ADR-0107、
    // 引擎侧同实/异实分辨拒收文案（#341 B）呼应：铸造前自我审查 + 撞名时分辨引导两头夹。
    version: 9, date: '2026-09-19', changeType: '铸名纪律（#341）：【单元命名】增四条（原子性/复合名三问/概念性判据/同形异义不并轨）+ concepts 参数说明补原子性 + 工具使用交叉引用',
    expectedDelta: '【单元命名】增四条：铸名原子性（犹豫默认拆——拆错有合并兑底、合错只能废弃+重铸且历史地址永久污染）、复合名三问（「与/和」并列必拆、未来 teaches/assumes 档位可能分叉的拆、学科公认单名保持）、概念性判据（有节点 assumes 才是概念，人物/作品/事件等展示性提及不铸名）、同形异义不并轨（撞名铸消歧名「基名（限定语）」）；draft_patch 的 concepts 参数说明句尾补「铸名须原子，一名只指一个可独立教学的对象」；【工具使用】第 2 条句尾补「铸就按【单元命名】的铸名纪律铸」（交叉引用，细则单源）。预期增量：随批铸名从「能用就铸的顺手动作」升为带拆合判断的纪律动作——复合概念拆分增多、单条概念粒度下降，撞名与事后合并提案减少；与受理门撞名分辨拒收文案（同实引用/收编/合并，异实消歧名）形成铸造前引导 + 撞名时纠正的两头夹。过门记录：check ✓（同提交登记）；replay ✓（0 件回归；教练执行 tool-calls 不在回放面，如实登记）；compare 未做真模型对照（同先例）',
  }, {
    // 教学编排纪律修订（第三轮测试归因 + 用户裁决后授权落笔）：把「慢就是快」与「构成分解
    // 读法」写进纪律面——前者给横向补实（补前置/拆分/回连）与向前新增同为推进的身份，
    // 后者掐断「罗盘第一条 = 下一站」的误读（规则侧根因）；入口真零基础校验封「隐性前置
    // 打包进入口」；并拢时机把终点.pre 线头堆积从信号接到 consolidate 动作；set_pre 一行
    // 定案多线语义（终点的 pre 随生长累积是常态，逐批接线是机械分类，与收尾宣告两回事）。
    version: 10, date: '2026-09-20', changeType: '教学编排纪律修订：慢就是快 + 入口零基础校验 + 反向盘点/并拢时机 + 罗盘构成分解读法 + set_pre 多线定案 + 批内梯度 + 一步一批',
    expectedDelta: '【构建准则】① 起始节点条补「按真实零基础校验：节点直接用到的每个概念，要么本节点就教它，要么是日常语言常识（例如负数、分数这类需要先教的概念，不得隐含打包进入口）」；② 新增「慢就是快」条（横向补实与向前新增同为推进——拆分大节点/补前置台阶/回连缺口都是实质推进）；③ 难度渐进条尾补「同批新节点不应全部落在同一难度档——批内也要有梯度，或分属不同构成项」；④ 罗盘条改写为「罗盘是对终点的专业说明：它把终点分解为并列的构成项，不提供生长顺序——顺序只由学习者认知链决定……条目对照是选择时的校验，不写入节点 note」；⑤ 新增「反向盘点」条（正向推进之外反向检查罗盘构成项要成立缺哪些地基概念/整块维度）；⑥ 新增「并拢时机」条（线头堆积 = consolidate 并拢信号）。【单元命名】teaches 条补「并随手为该概念带一条误解先验（misconceptions，一条即可；在册已有则不重复）」。【补丁纪律】set_pre 行整句重写为多线语义（终点.pre = 各条在长的线当前最深的节点；接线是机械分类；收尾宣告另由纯 set_pre 独立批完成）。【弧对齐】serves_arc 补「只是给罗盘站的参考信息，拿不准归属就不写」；段尾「两件都在 draft_arc 上给（至少给一件）」改为「draft_arc 完全可选且应当少用」。 【结束条件】批义务计数澄清（新增/补前置台阶/拆分大节点都计入）+ 「面板『生长一步』下发的会话恰发布一批即 finish 收束——一步是一步」。预期增量：① 入口节点不再打包未教概念（实测第二轮「整数」定义内含负整数、古典概型当概率线入口）；② 补实动作获得推进身份，批义务不再只认新增（跳步的动机面缓解）；③ 模型不再按罗盘条目排列位次推进（实测第二轮每会话一次「罗盘说微积分、常识说零基础」内耗）；④ serves_arc 归属纠结大幅减少（实测第三轮推理爆量主因之一，单轮 17.5k 推理含整段归属自辩）；⑤ 终点.pre 维持多线形态不再塌缩为单线（v2 三条→v3 一条的实测退化）；⑥ 一步的产量由门执法（GROWTH_STEP_NODE_CAP 同批落地）而非模型自觉。过门记录：check ✓（同提交登记）；replay ✓（0 件回归；教练执行 tool-calls 不在回放面，同本站 v1–v9 先例）；compare 未做真模型对照（同先例）',
  }, {
    // 同版本号下的注入面登记（先例见教练执行 v8 第二条）：模板文本之外的最终 prompt 面
    // ——写件工具描述、首级判据、审计 findings 文案、读件视图。不被提交级门执法
    // （ADR-0072 §已知边界 ④），靠人审 + 本登记兜。
    version: 10, date: '2026-09-20', changeType: '注入面四件（模板文本未动）：隐性前提对照取材扩展 + draft_arc 描述收窄 + 首级判据词汇域扩罗盘条目 + 活跃线视图',
    expectedDelta: '① draft_audit 工具描述与 draft_patch 的 audit findings 口径补「隐性前提对照」第三类：本批节点名、note 与铸名概念 definition 中出现、却既不在任何 teaches/assumes 也未随批铸名的领域术语，逐个列入 findings（落实出口 = 补前置台阶接线/回连在册概念/显式声明首引三条，与概念反查门同构）。扩展点在 definition——「整数」的定义含「负整数」这类概念定义内打包，是全字面匹配与 teaches/assumes 对照的共同盲区（实测第二轮事故形态）。② draft_arc 工具描述删「至少给一件」义务，改为「完全可选且应当少用」。③ COACH_FIRST_RUNG_CRITERIA 的「本课程词汇」域从「课程名与终点锚」扩为「课程名、终点锚，以及罗盘『剩余路线』各条目所指能力域内的具体事物（取域内的具体可学对象，不是照抄条目名）」——几何/空间类词汇从此合法出现在第一级台阶。④ graph_view 与上下文包新增「活跃线」段：终点接线的线头经上游闭包滤出真前沿（被更深线消费的旧头滤除），逐行给线头/深度/首环 pre——终点.pre 线头堆积从字面长度变成可视读数（并拢与补弱的判读面），读侧派生零落盘。⑤ FIND_SEAL_TODO 文案中性化（「已铺通待收尾」→「已有接线……本条仅提示收尾通道；未铺通则忽略本条继续当回合计划」——旧文案的「就该继续生长」在零思考档下被逐轮照办，实测一次点击铺出 33 节点的事故引信）；FIND_CONFUSABLE_DANGLING 文案补「既不在册也非本批铸名」，与同批修复的校验口径（resolveConcept 并入本批铸名）一致。预期增量：入口打包（定义内隐性前提）进入审计行动清单；首级判据不再把几何类词汇挡在起点之外；教练每批有现成的线头读数（不必再靠推理自行盘点）；审计 findings 不再出现「继续生长」行动指令与同批铸名的假悬空。过门记录：check ✓（同提交登记）；replay ✓（0 件回归）；compare 未做真模型对照（同先例）',
  }],
}
