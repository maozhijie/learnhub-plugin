...
3. 附有「块工作表」（覆盖锚定课程）时：阶段按工作表块组织，顺序服务于终点综合；块清单是完成判据的核对表，路线不复制它的承诺语义。
4. 附有「学习者批注」时：它是软输入——提议非指令；与你的判断冲突时保留你的路线，并在受影响阶段行尾以（批注：…）回应一句。
5. 能力锚定课程：阶段服务于终点掌握（前置闭包怎么长、能力怎么合成），不罗列教科书目录。

## 输出

只输出「剩余路线」一节的正文（markdown，不带 "## " 标题、不带代码围栏、不要解释）：3–7 个阶段条目，每条一行，格式 \`- **阶段名**：一句话（这个阶段为什么朝终点推进）\`。
`,
...
4. route 恒写：罗盘「剩余路线」随批重写（3–7 条阶段条目，非承诺措辞——不写时间估算与进度承诺；路线只是草图）。
...
7. 接线义务与收尾批（#198 / ADR-0055）：前进/换向批含 add_node 时必须接线终点（见算子集）；既有节点已满足终点要求、你裁决停摆之前，若终点.pre 尚未指向你认定的最终台阶，须先出一条**零 add_node 的纯 set_pre 接线批**（ops 只含 set_pre { node: <终点>, pre: [最终台阶] }）再停摆——收尾即宣告承诺兑现（apply 会往终点锚写 sealed 收尾宣告；之后重开含 add_node 的主线接线批时自动清除）。终点只可被 set_pre 接线，禁止出现在任何 add_node 的 pre 里（禁长过目标）。
