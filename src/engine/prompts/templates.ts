...
`,
    ...
    罗盘初画: `\
<!-- learnhub:prompt/v4 -->
# 罗盘初画提示词（罗盘站；用户可编辑；终点锚、当前图与学习者批注由系统附在本模板之后）

你是 learnhub 学习系统的领域大师。这门课的方向由学习者手加的终点锚声明（可能多个），终点锚的目标描述是一句**程度声明**——终止在什么程度（深度/广度到哪档）。请为每个终点各画一节「剩余路线」：从学习者当前的水平到那个程度状态，中间要长出哪些**他能做的事**。它是常驻可见的地形感，不是承诺，也不是权威结构；此后由罗盘站在真实方向变化时低频重画。
...
只输出「剩余路线」一节的正文（markdown，不带 "## " 标题、不带代码围栏、不要解释）：每个终点一节——节头一行 \`- **终点名**：\`，节下 3–7 个阶段条目，每条一行，格式 \`- **阶段名**：一句话（推进程度声明的哪个维度）\`。
`,
    ...
    罗盘重画: `\
<!-- learnhub:prompt/v1 -->
# 罗盘重画提示词（罗盘站重估族；用户可编辑；终点锚、当前图与学习者批注由系统附在本模板之后）
...
`,
    ...
    // v3（#316 / ADR-0099）：撤 route——罗盘写权反转为罗盘站独占，思路官不再写弧；
    // v3（#313 C15 已并入）：非停摆算子必须给台阶，停摆只能由 operator: 停摆 表达。
    // v4（#319 / ADR-0099）：建议与对齐结构化——增 serves_arc（软对齐指认，值为弧的
    // 阶段标题）与 repaint_suggest（结构性重画建议，理由枚举收窄为结构性事由）。
    思路官回合: `\
<!-- learnhub:prompt/v4 -->
...
裁决只依据附后的材料。粒度变焦（具体节点名、逐跳拓扑、概念逐字对表）是执行官的事——你的计划**零节点名、零图上引用**：用「意图句」描述每个台阶（学习者学完能做什么、为什么朝终点推进），不写图上已有什么名字；概念面只能写**概念名**（teaches_concept），且必须逐字来自上下文包的登记表档位区。
...
收尾朝向是算子判断的一部分：某终点的坡道已铺通、无需新节点时照常给方向——收尾宣告由引擎对纯接线批自动完成（批形态触发的引擎自动标记），不归你也不归执行官另立仪式；这个方向照旧要给至少一个台阶（写「补齐最后台阶的接线意图」即可，非停摆算子不给空 steps）。
...
recheck:
  ...
  days: 10
serves_arc: <本计划服务弧的阶段标题（可选；照抄罗盘「剩余路线」条目的粗体名）>
repaint_suggest:
  reason_class: 前沿枯竭|弧段走完|终点变更
  note: 一句话（可选）

停摆只能由 operator: 停摆 表达——其余算子必须给出至少一个台阶（steps 空 = 计划不完整，门会拒收）；recheck 仅 operator=插入 时携带，其余算子禁带。罗盘「剩余路线」不归你写：弧的写权在罗盘站，你对弧只有建议权与对齐指认——serves_arc 可选，只在你确知本计划服务弧的哪一阶段时写，值必须是罗盘「剩余路线」里的阶段标题原文（不是节点名）；不确定就省略，不硬凑。repaint_suggest 可选，且只允许结构性事由（前沿枯竭 = 朝声明终点的前沿已无可补台阶；弧段走完 = 当前弧段全部走完需下一段；终点变更 = 终点声明或目标描述类事件）；学习者的作答、答错、停滞、保留率这类读数信号不构成重画理由——战术波动不推翻战略。
`,
    // 思路官重裁（#273 显式重裁族）：node_skip / panel_dispatch 触发的显式重新裁决——
    // 上一次裁决摘要随包回灌，可沿用可推翻。裁决语义与「思路官回合」同构（两族分键
    // 只为用户可分别编辑，#273 票面）；收尾朝向不是提示词族。
    // v3（#316 / ADR-0099）：与「思路官回合」同步撤 route——写权反转为罗盘站独占。
    // v4（#319 / ADR-0099）：与「思路官回合」同步增 serves_arc 与 repaint_suggest。
    思路官重裁: `\
<!-- learnhub:prompt/v4 -->
...
你是 learnhub 学习系统的教练执行官。思路官的裁决不在本站——你自足地读附后的上下文包、图面与草稿状态，把下一步生长以**批量补丁**写进生长草稿，并在批完成后按批发布（finish）。生长草稿是与受理门同一套校验的暂存区：草稿通过 = 门通过；发布前它既不是活图也不是提案。
