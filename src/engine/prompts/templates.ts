...
- 难度渐进：相邻前后置节点的 difficulty 步进不超过 1；不连续铺两个 difficulty ≥3 的节点——中间夹一个巩固/低档节点消化； difficulty ≥3 的硬节点 est 按其真实体量给（45–60 分钟），不为凑「30 分钟量级」而砍内容或抬高前置假设。
...
- **前进**：沿终点方向给前沿补下一步台阶，服务终点前置闭包的合成；批内含前进条目时必声明朝向（target_endpoints，优先选能同时推进多个未达成终点次选同时推进多罗盘个的方向，交汇优先）。
- **插入**：卡点集中度/停滞指向缺口时补过渡台阶；插入条目必须携带复诊预注册（recheck）。
- **巩固**：足迹末端接综合收束，只行使已教概念。
- **旁支**：为讲清主线必须先教的支线台阶。
- **换向**：学习者批注/目标指向变化时重定方向；批内含换向条目时与前进同样声明朝向。
- **停摆**：结构暂无需变化——本回合不产结构，用 draft_note 写明理由。

算子是**逐条目属性**：每条 add_node 用 operator 声明自己的算子，同一批可以混算子（前进铺主线 + 插入补台阶 + 旁支挂支线，一次长出一组各种类别的节点）；混批时接线义务只跟前进/换向条目、复诊只跟插入条目。
...
- 每条 add_node 用 operator 声明自己的生长算子（前进/插入/巩固/旁支/换向），批理由用 note_reason 一句话声明；前进/换向条目所在的批必须声明 target_endpoints 并对声明的终点 set_pre 接线（终点禁出现在 add_node 的 pre 里）；插入条目用 recheck 写复诊预注册。
