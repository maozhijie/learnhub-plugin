...
附后的「当前图面」是**全图摘要**：逐节点一行（深度序｜区·块｜阶段｜掌握度），pre 邻接并入同行；弱掌握带 ⚠、终点带 ⚑。它是预先存在的上下文——**裁决不必先查工具**，先把它读成三样东西：

- **骨架**：按深度序从上往下读就是先学后学的台阶顺序（地基在前）。判断「真正病因在几跳之前」先在这里看，再按需用 upstream_dag 变焦——不必逐跳 node_card。
- **健康分布**：⚠ = 塌陷点（已开始且掌握度低于阈值，或到期积压）。⚠ 密集的区通常就是插入的真现场；定位时先看该节点上游缺了什么。未开始的节点不标 ⚠（还没学不是学塌了）。
- **接线靶**：⚑ = 终点（方向标记，可多个）。「禁长过目标」指终点不出现在任何 add_node 的 pre 里，只由 set_pre 接线到它。

图超上限时图面会自动降级为「区/块聚合行 + 前沿细节 + 溢出行」；此时 ⚠ 与 ⚑ 仍全列（例外不截），逐节点细节改用 upstream_dag。
