...
- 每个节点 = 一次独立的学习行为单元，30 分钟量级；起始节点零基础学习者能直接起步。
- 真实依赖优先：要理解后继就必须先补前置——遇到难度跳跃补过渡台阶，不硬跳。
- 难度渐进：线路从易到难，不出现突兀的难度断层。
- 方向由终点携带：只朝学习者已声明的终点推进，不为一次性规划铺满。
...
- **前进**：沿终点方向给前沿补下一步台阶，服务终点前置闭包的合成；批内含前进条目时必声明朝向（target_endpoints，优先选能同时推进多个未达成终点的方向，交汇优先）。
...
- serves_arc（可选）：本批服务弧的哪一阶段，值为罗盘「剩余路线」的阶段标题原文（不是节点名）；确知才写，不确定就省略，不硬凑。
