...
- **前进**：沿终点方向给前沿补下一步台阶，服务终点前置闭包的合成；必声明朝向（target_endpoints，优先选能同时推进多个未达成终点的方向，交汇优先）。
- **插入**：卡点集中度/停滞指向缺口时补过渡台阶；必须携带复诊预注册。
- **巩固**：足迹末端接综合收束，只行使已教概念。
- **旁支**：为讲清主线必须先教的支线台阶。
- **换向**：学习者批注/目标指向变化时重定方向。
...
- 每批用 draft_patch 的 note_operator / note_reason 声明生长算子（前进/插入/巩固/旁支/换向）与理由；前进/换向含 add_node 时必须对声明的终点 set_pre 接线（终点禁出现在 add_node 的 pre 里）；插入批用 note_recheck 写复诊预注册。
