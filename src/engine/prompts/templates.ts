...
`,
    ...
    // v2（#313 C15，同版本第二条）：「steps 为空 = 停摆」与算子枚举自相矛盾——非停摆算子写空 steps 会被门拒
    // （计划不许「看起来像停摆」），停摆只能由 operator: 停摆 表达。
    思路官回合: `\
<!-- learnhub:prompt/v2 -->
...
停摆只能由 operator: 停摆 表达——其余算子必须给出至少一个台阶（steps 空 = 计划不完整，门会拒收）；recheck 仅 operator=插入 时携带，其余算子禁带；route 省略 = 不改写旧稿。
`,
    ...
    // 两族一致，措辞按站名指代各写各的（同一条计划契约，分键只为用户可分别编辑）。
    // v2（#313 C15，同版本第二条）：与「思路官回合」同款收紧——非停摆算子必须给台阶。
    思路官重裁: `\
<!-- learnhub:prompt/v2 -->
