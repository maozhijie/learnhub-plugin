...
- **Land it yourself — do not stop at a pushed branch**: closing the tickets is not the end of the flow. Landing means "on the branch this work belongs to", **not** "on a branch you created": a single session working directly in the main checkout lands by committing and pushing on the current branch — commit → push → close the issue, nothing to merge back. **Cutting a branch is never a requirement**; it is an isolation measure for parallel sessions (see `parallel-sessions.md`). Only when the session does run on its own delivery branch does landing additionally mean merging that branch back into the branch it was cut from (normally main), verifying (full test suite + build in the source checkout), and cleaning up its worktree — without waiting for the user to ask for the merge. Step-by-step: `docs/agents/parallel-sessions.md` → "Merge back".
