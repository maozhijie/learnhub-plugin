...
**两门都看不见的一类（已知边界，如实登记，别当成门能拦住的形态）**：**同版本号下的 prompt 变化**——改模板文本而不 bump、改拼装缝（`prompt-assembly.ts`）、注入面增删。提交级门的判据是**版本号集合差**、状态级门的判据是**版本号对齐**，两者都只看版本号，这类变更两门都不红。它靠**人审 + 上面的登记纪律**兜（登记面是同一版本号下的第二条/第三条条目，先例见 ADR-0065 §6、ADR-0071 §后续）；要把它机械化需要「哪些键的模板文本变了」的键级判据（解析 `PROMPT_KINDS` 模板字面量），代价与脆性都高，本票不做（ADR-0072 §已知边界 **④**——③ 是「评审对照需要真模型」，本条此前误引为 ③，随 #302 更正）。
