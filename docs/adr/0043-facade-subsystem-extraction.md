...
① **窄面三向一致门的第三方向（#166 已落地）**。`scripts/scan-deps-face.mjs` 原先只比「deps 声明 ↔ 类体 `this.e.X` 实用」两个方向（实测两向今天都是 0：无死成员、无未声明使用，故工具**退出 0**），而**门面 `new XSubsystem({...})` 提供的键**这一方向当时缺失——多余接线 **30 条**（声明 167 ／ 接线 **197**；本 ADR 原记的 188 系早期手数口径，#166 落地时由门实测修正，见 ADR-0047 的「落地实测」条），其中 **10 条 phantom**（`this.X` 在门面上不存在，全在 `growth-subsystem.ts` 的接线里）当时**完全不可见**。补该方向时**只取接线字面量的 brace-depth-1 键**：`ChannelsSubsystem` 的接线里 `registry: { load, loadNoteSources, save, get }` 是嵌套窄子面（`ChannelsDeps` 正以结构化窄面声明它），按扁平正则抽取会把子面成员误计为顶层接线——实测会伪造出 4 条不存在的 phantom。门与自检见 ADR-0047。

② **窄面宽度的声明预算（handles ≤12 / facade ≤20 / fns ≤10）在顶层重划落地前是非活动目标**，活动门是逐子系统实测基线（#166 落地实测最大 handles **9** / facade **20** / fns **1**——本 ADR 原记的 ≈10／≈14／≤3 系手数；按槽位定义 facade 已触上限 20）。理由：三槽位分组会改成员的槽位归属，用旧形状的阈值绑定新边界正是要避免的事。

③ **枢纽领主判定线（领主文件 importer ≥5 个 engine 模块）与「零既有模块搬移、接缝路径不晃」原则不变**，但文件归属若因重划而变，规模与宽度阈值**需重推**（不是就地调）。

④ **「聚合+转发」是过渡形态，不是终态（ADR-0049 补记，2026-09-11）**。本 ADR 的转发层是为**拆分期的机械搬移**服务的——理由是「签名不动、调用点不动、每刀可独立回滚」。九个子系统抽出后该价值即用尽，剩下的 **199 个转发体**（占门面 700 行）是纯转发税。终态由 **ADR-0049** 定案：hub 降级为纯容器、公开面改 `engine.<子系统>.<方法>`、转发体删除（施工票 #182，立而不排期）。**同时更正一处读法**：本 ADR 与 ADR-0044／0045／0048 里出现的「**公共面零改动**」是**逐刀纪律**（各自限定在那一票的操作范围内，为的是每刀可独立回滚），**不是不变量**——本 ADR 自己的否决理由写的是「违反增量纪律（**一刀**动全部消费方）」，那是"别一刀切完"，不是"永远不许"。本 ADR「门面的价值是公共面的稳定」一句中的"公共面"应读作**产品契约**（HTTP 路由路径与响应形状、agent 工具名与 schema、磁盘文件格式），**不含引擎门面签名**——门面签名是内部接缝，正是 ADR-0049 要重构的对象。

以上①–③的机制细节、棘轮刚性与门清单见 ADR-0047。
