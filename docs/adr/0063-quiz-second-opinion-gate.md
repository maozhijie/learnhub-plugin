...
- **不一致处置**：该题拒收，拒收原因 + 独立解 + 存储键回灌修复轮（gateRepairRound 形态：**恰一次**，题目生成站 repair 形态、deep 档）；修复题原位替换后**再审计一次**，一致才入库；修复轮产出不可用或仍不一致 → **弃题不阻塞整批**（弃题进 rejected 报告面，理由带「第二意见」前缀）。**修复轮是批级单调用**（一批不一致题合一次回灌，省逐题调用成本）：修复产出解析失败/条数不齐时这批题一体弃（题级粒度让位于成本与映射可靠性——部分采纳的顺序映射不可信）。个别弃题不连坐其余题；**全弃光时沿用既有「一道都没入库」fail loud 语义**（零幸存 = 出题失败可重试，与逐题门全拒同口径，不算「阻塞」——那批题本就无一可入库）。
...
- 拒收/弃题走 rejected 报告面：个别弃题不连坐其余题；全弃光是「这道题全不成立」的诚实失败（fail loud 可重试），不是第二意见门新增的阻塞形态。
- 代价：出题成本上升（抽样率可调可关）；solver 误判会造成误弃题——由「修复轮改好则入库」+ 量规回流治理，不在门上放松比较器。
- `evaluateAllo` 判卷与对账共用同一比较器：判卷语义修订时两处同步，对账口径永不漂移于判卷口径。
- 注册表联动（ADR-0061）：题目生成站的修复策略列随本门更新（审计修复恰一次 ≠ 门错整批修复轮，后者仍归 #217 评估）；「独立解题」作为新站进输出契约注册表与语料站词表，锚门的站名对账升级为双向（词表每站必须在册或明示不在册）。
