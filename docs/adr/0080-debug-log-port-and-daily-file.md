...
## §修订（2026-09-17，#313 B6：点名的事件必须是真发得出来的）

**上文「两条一眼判据」的一半是假的**：`coach.repair.trigger` 全仓零发出点（`git grep` 只命中本 ADR、门册与一处代码注释），照它去 grep 日志会得到「没跑过重裁」的**假否定**——正是本 ADR 开头要治的那类病，只是这次坏在文档侧。同一条里还有三个幽灵成员：`coach.segment.enter｜exit`、`coach.round.apply_fail`（同为闭集成员，全仓零发出点）；反过来 `coach.plan.enter｜exit｜reinject` 自 #273 两站拆分起一直在发，却从未登记。ADR-0091 附录是**只增不改**的增量登记处，本次退役与补登都记在那里；此处只更正两点指针：

- 「有没有跑过重裁/重试」的主判据 = **`coach.gate.reject`**（WARN，`station` 区分站、`gate` 区分门、`round`/`fatal` 区分首轮与两轮死因；明细进续行）。思路官计划门判成之后紧接着发的 `coach.plan.reinject`（DEBUG）是修复轮回灌的逐轮轨迹面，不是主判据（默认 INFO 档看不见它）。
- 闭集里 `coach.segment.*` 一族的实际形态是 `coach.plan.*`（两站拆分的产物，见 ADR-0091 补登）。

不改上文闭集清单：它是**当票快照**，退役与新增都归 ADR-0091 附录（本 ADR 自己定的规矩）。
