...
**更正上「唯一牵动路径键控门的是 ⑦」**：实测刀①–④ **每一刀**都触到，全量 7 类（刀⑤–⑧ 复用同表，只多第 8 类，见 §修订二）：
...
**施工器**：`scripts/move-engine-domain.mjs`（刀① 沉淀，刀①–⑧ 全程复用，已入册 architecture.md 单跑脚本清单）——`git mv` + 两向 specifier 改写（未搬文件指向被搬文件的 + 被搬文件自身全部相对 specifier 重算；后者不可省，否则被搬文件指向留守文件的 specifier 静默失配）。

## §修订二（2026-09-16，#306/#307 刀⑤–⑧ 实测：施工完成）

八刀落齐，顶层 91 → **5**（`index`／`types`／`views`／`store`／`data-check`）。⑤⑦⑧ 触到的路径键控面**与上表同构**（逐刀落点见门册），但实测**多出第 8 类**。三条补充供复用：
