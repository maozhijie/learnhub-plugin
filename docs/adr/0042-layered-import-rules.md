...
**与顶层分层的关系（ADR-0044 / ADR-0047 补记）**：R1–R7 **不废止、不重写**，升格为 ADR-0044 四层责任（域／应用／适配器／投递）在当前文件布局下的**导入方向执法**——R1 是投递→应用、R2 是应用→适配器（也是「engine 禁 import 宿主」的原始字面）、R3 是应用与适配器之间的 dsh 耦合分界、R6 是应用层内部「门面是唯一汇点」、R7 是应用层内零环。ADR-0044 在其上补的是**命名与一层缺口**（适配器仍住在 `src/engine/`：33 个文件 import `node:fs`、284 处调用、`new Date(` 35 处散在 14 个文件、`Math.random` 4 处），不含规则改写——四层不引入新目录树，`src/host/` 即适配器的外移目的地，故 R2 自动成为那条边界。本 ADR 的七条规则何时收紧／放宽，归 ADR-0047 的棘轮时序管辖：**顶层重划落地前只作棘轮、只降不升**。
