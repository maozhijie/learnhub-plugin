...
- `ui/` 的 151 处类型错与它自己的 tsconfig／vite 构建**不在本 ADR 范围**。
  - **注（2026-09-12，#184）**：「151 处」是本 ADR 落笔时的历史读数，与本文件上方「#169 落地记录」的「ui/src 类型错 58 → 0」并存自相矛盾；以落地记录为准——#178 批4 后依赖侧存量债亦清零，现状看 `npm run typecheck` 输出与 `scripts/arch-baseline.json`。原句保留不改。
