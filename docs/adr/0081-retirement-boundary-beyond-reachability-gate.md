...
理由：与 ADR-0080 同族——**「声明即产品面」**。无产品入口的命令/路由/引擎方法不该继续养着：维护者要记它、契约测试要覆盖它、快照条目还会把死入口**钉活**。行为快照是**行为回归**（这条路由当时该怎么回），不是**可达性回归**（还有谁在调它）；把「快照里有 200」读成「入口还活着」是把回归网误当可达性门。退役的判据是**可达性**（有没有产品入口/调用方），不是快照存在与否——这正是本 ADR 与父票 #254「死入口退役」主题的边界裁定。
...
- **保留命令/路由、只删 UI**——死入口继续养（引擎方法、视图类型、快照条目、契约测试），问题没解决；与 ADR-0080 否决的「只删面板路由」同形。
...
取号：0081（写前 `ls docs/adr/` 确认，0080 已占）。票面：#256（父 #254）。
