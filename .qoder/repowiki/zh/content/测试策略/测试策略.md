...
- [tests/stuck-report.test.ts](file://tests/stuck-report.test.ts)
- [tests/stuck-report-ui.test.ts](file://tests/stuck-report-ui.test.ts)
- [src/engine/stuck-report.ts](file://src/engine/stuck-report.ts)
- [ui/src/components/StuckReportEntry.tsx](file://ui/src/components/StuckReportEntry.tsx)
</cite>

## 更新摘要
**变更内容**
- 新增卡点自报系统测试套件章节，涵盖三层测试架构（纯函数、引擎集成、宿主编排）
- 扩展测试辅助工具部分，增加与卡点自报相关的夹具和探针
- 更新测试分层策略，体现新功能的多层测试覆盖
- 增强故障排查指南，包含卡点自报系统的调试技巧
...
**更新** 新增了卡点自报系统的完整测试套件，包括纯函数行为验证、引擎集成测试、宿主编排场景测试和UI组件测试，体现了现代前端应用的多层测试策略。
...
- **新增** 卡点自报系统测试套件，包含 `stuck-report.test.ts`（引擎与宿主集成测试）和 `stuck-report-ui.test.ts`（UI组件测试）。
...
D --> F["src/*<br/>被测模块"]
B --> G["tests/stuck-report*.test.ts<br/>卡点自报测试套件"]
G --> H["src/engine/stuck-report.ts<br/>纯函数逻辑"]
G --> I["ui/src/components/StuckReportEntry.tsx<br/>UI组件"]
```
...
- [tests/stuck-report-ui.test.ts:1-98](file://tests/stuck-report-ui.test.ts#L1-L98)
...
- **新增** 卡点自报测试夹具：提供频控验证、消费折叠、注入块生成等专用测试工具，支持三层测试架构。
...
- **新增** 卡点自报系统采用三层测试架构：纯函数层（频控逻辑）、引擎集成层（数据存储与查询）、宿主编排层（API处理与任务调度）。
...
participant SR as "StuckReportSystem"
T->>V : 传入选项注册表/图/笔记/题库/时钟/rng
V->>FS : 写入课程注册表、图数据、笔记、题库、state
V->>R : 注入 clock/rng/provider/model 等
V->>E : 构造引擎实例
T->>SR : 测试卡点自报功能
SR->>E : 调用 stuckReportAppend/stuckPending
E->>FS : 读取/写入 practice 流水
E-->>T : 返回结果含频控状态/待消费清单
```
...
- [tests/stuck-report.test.ts:119-168](file://tests/stuck-report.test.ts#L119-L168)
...
- **新增** 卡点自报纯函数测试：验证频控逻辑（同节点每日限制、全课程总量限制）、消费折叠（精确到行ID的消费标记）、注入块生成（原话保真与换行转义）。
...
- [tests/helpers/vault.ts:1-233](file://tests/helpers/vault.ts#L1-L233)
- [tests/stuck-report.test.ts:37-104](file://tests/stuck-report.test.ts#L37-L104)
...
- **新增** 卡点自报引擎集成测试：验证数据存储（practice 流水追加）、频控验证（fail loud 带原因）、消费标记（幂等性）、与作答投影互不可见（零激励影响）。
...
participant SR as "StuckReportEngine"
...
SR->>ENG : stuckReportAppend/stuckPending
ENG->>ENG : 频控验证/数据存储
ENG-->>SR : 返回操作结果
```
...
- **新增** 卡点自报端到端场景：验证从用户提交到教练回合执行的完整流程，包括频控拒绝、在途合并、消费标记等复杂场景。
...
S->>SR : 测试卡点自报全流程
SR->>E : stuckReportAppend → coachGrowthBatch
E->>FS : 写 practice 流水/消费标记
S-->>S : 断言文件与状态一致性
```

**图表来源**
- [scripts/e2e.mjs:1-695](file://scripts/e2e.mjs#L1-L695)
- [tests/stuck-report.test.ts:237-313](file://tests/stuck-report.test.ts#L237-L313)
...
- **新增** 卡点自报UI组件测试：验证 StuckReportEntry 组件的展开/收起、多行输入、提交按钮、回执呈现（成功/在途/拒绝三种状态）、语义锁（canonical词汇检测）。
...
F --> R["测试路由表"]
R --> U
U --> SR["StuckReportEntry 组件"]
SR --> API["api.stuckReport 调用"]
API --> ROUTE["/coach/stuck-report 路由"]
```

**图表来源**
- [tests/helpers/ui-dom.ts:1-129](file://tests/helpers/ui-dom.ts#L1-L129)
- [tests/helpers/tsx-loader.mjs:1-66](file://tests/helpers/tsx-loader.mjs#L1-L66)
- [tests/stuck-report-ui.test.ts:29-98](file://tests/stuck-report-ui.test.ts#L29-L98)
...
### 卡点自报系统测试架构
**新增** 卡点自报系统采用三层测试架构，确保功能的完整性和可靠性：

#### 纯函数层测试
- **频控逻辑**：验证同节点每日限制（1条）和全课程总量限制（5条/日）
- **消费折叠**：测试消费标记的精确匹配（按行ID而非时间戳）
- **注入块生成**：验证原话保真、换行转义、已消费过滤

#### 引擎集成层测试  
- **数据存储**：验证 practice 流水追加、course 归一化、与原话逐字保存
- **频控验证**：测试 fail loud 机制、图外节点拒绝、与作答投影互不可见
- **消费标记**：验证幂等性、pending 列表清空、异常时留账不丢

#### 宿主编排层测试
- **API处理**：验证 POST /coach/stuck-report 路由处理、回执三态（成功/在途/拒绝）
- **任务调度**：测试 force 回合入队、在途合并、消费标记时机
- **异常处理**：验证回合失败时的自报保留、频控拒绝的错误处理

```mermaid
graph LR
subgraph "纯函数层"
A["stuckReportGate<br/>频控逻辑"]
B["foldStuckReports<br/>消费折叠"]
C["stuckReportInject<br/>注入块生成"]
end
subgraph "引擎集成层"
D["stuckReportAppend<br/>数据存储"]
E["stuckPending<br/>待消费查询"]
F["stuckMarkConsumed<br/>消费标记"]
end
subgraph "宿主编排层"
G["handleApi<br/>API处理"]
H["coachGrowthBatch<br/>任务调度"]
I["jobs<br/>队列管理"]
end
A --> D
B --> E
...
**图表来源**
- [src/engine/stuck-report.ts:24-84](file://src/engine/stuck-report.ts#L24-L84)
- [tests/stuck-report.test.ts:22-313](file://tests/stuck-report.test.ts#L22-L313)
...
- **新增** 卡点自报测试套件依赖 engine/stuck-report.ts、engine/growth-subsystem.ts、host/handlers.ts、ui/components/StuckReportEntry.tsx 等模块。
...
SR["tests/stuck-report*.test.ts"] --> SRM["src/engine/stuck-report.ts"]
SR --> SRG["src/engine/growth-subsystem.ts"]
SR --> SRH["src/host/handlers.ts"]
SR --> SRUI["ui/src/components/StuckReportEntry.tsx"]
```
...
- **新增** 卡点自报系统性能测试：可通过批量提交自报请求测试频控机制的性能表现，验证在高并发场景下数据存储和查询的效率。
...
- **新增** 卡点自报系统故障排查：
  - 频控拒绝：检查 prior 数据是否正确获取，确认学习日计算和节点归属
  - 消费标记异常：验证行ID生成逻辑（ts|node格式）和消费标记的精确匹配
  - UI组件状态：确认回调处理是否正确，检查 receipt 状态的样式类名
  - 异步竞态：使用 until 等待机制确保任务状态稳定后再断言
...
- [tests/stuck-report.test.ts:172-180](file://tests/stuck-report.test.ts#L172-L180)

## 结论
本项目的测试体系以"隔离、可重复、可观测"为核心，通过 withVault、tools-probe、ui-dom 等夹具与探针，覆盖了从单元到端到端的完整质量保障链。**新增的卡点自报系统测试套件**展示了现代前端应用的多层测试策略，包括纯函数验证、引擎集成测试、宿主编排测试和UI组件测试，确保了功能的完整性和可靠性。建议在生产环境中延续这些实践：严格使用夹具构造测试环境、保持快照基线更新、在 CI 中并行执行测试、对关键路径保留 e2e 脚本，并对 UI 交互增加路由桩与 DOM 测试。
...
- 测试分层
  ...
  - **新增** 多层测试架构：对于复杂功能（如卡点自报），采用纯函数层、引擎集成层、宿主编排层的分层测试策略。
- 夹具与工具
  ...
  - **新增** 卡点自报专用夹具：提供频控验证、消费折叠、注入块生成等测试工具。
- 模拟与存根
