...
- 数据体检：只读扫描 Vault，分类 Missing/Broken/Archived/Hint，产出报告与库存统计。
...
## 结论
本指南基于代码实现整理了安装、配置、运行、数据一致性与性能等方面的常见问题与解决方案。通过统一的参数校验、运行日志、健康检查与数据体检，能够快速定位与修复故障。建议在日常运维中结合健康分与数据体检报告，持续优化内容与系统稳定性。
