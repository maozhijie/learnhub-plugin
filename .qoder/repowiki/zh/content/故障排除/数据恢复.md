...
## 更新摘要
**所做更改**
- 增强了数据体检系统，新增孤儿笔记检测能力
- 将医生功能合并到数据检查系统中，提供统一的诊断能力
- 孤儿笔记（引用不存在节点的笔记）现在作为提示级发现被检测
- 遵循用户笔记始终是有效对象的原则，即使引用节点缺失也不判为损坏
...
## 核心组件
- 数据体检（只读盘点）：扫描注册表、图、笔记、题库、笔记源镜像、我的卡/错误卡、概念登记表、终点锚、存档区、边实验账本、提案、生成任务、追加流水等，输出 Missing/Broken/Archived/Hint 四类发现。**已增强**：新增孤儿笔记检测，将医生功能的未知格合并入统一诊断体系。
...
- **已增强** 关键行为：
  ...
  - **新增** 孤儿笔记检测：frontmatter 合法但节点名不在图节点集内 → hint finding（不进 status），定位到笔记路径与悬空节点名。
...
ScanReg --> OrphanCheck["孤儿笔记检测"]
OrphanCheck --> ScanSources["扫描笔记源镜像与清单"]
...
### 孤儿笔记检测机制
**新增功能** 数据检查系统现在能够检测孤儿笔记，这是医生功能合并到数据检查系统的重要特性：

- **检测逻辑**：在扫描笔记时，检查笔记的 frontmatter 中的节点名是否存在于当前图的节点集中
- **处理原则**：遵循 ADR-0004 原则，用户笔记始终是有效对象，即使引用节点缺失也不判为损坏
- **发现级别**：孤儿笔记以 hint 级别浮出，不进入整体状态评估（status）
- **定位信息**：提供精确的笔记路径和悬空节点名，便于人工处置

```mermaid
flowchart TD
NoteScan["扫描笔记"] --> ParseFM["解析frontmatter"]
ParseFM --> Validate{"frontmatter合法?"}
Validate -- 否 --> MarkBroken["标记为broken"]
Validate -- 是 --> CheckNode["检查节点是否在图内"]
CheckNode -- 否 --> MarkOrphan["标记为hint(note_orphan)"]
CheckNode -- 是 --> Next["继续扫描"]
MarkOrphan --> Next
MarkBroken --> Next
Next --> Done["完成"]
```
...
图表来源
- [data-check.ts:240-247](file://src/engine/data-check.ts#L240-L247)
...
当前可用的手动修复工具：
- 数据体检：通过 engine.dataCheck() 接口进行全库数据一致性检查，**新增孤儿笔记检测**
...
- **新增** 孤儿笔记检测增加了节点名查找操作，但通过 Set 数据结构保持 O(1) 查找效率。
...
- **新增** 孤儿笔记问题：
  - 现象：dataCheck 报 note_orphan hint finding，定位到特定笔记路径。
  - 处置：删除该笔记或将节点名称改回图内存在的节点名。
- 终点锚悬空：
...
## 结论
本系统通过"只读体检 + 原子写 + 断点恢复"的组合，实现了高可靠的数据一致性与可恢复性。**已增强的** dataCheck 提供全库可见性，包括新增的孤儿笔记检测能力，store/io 保障写安全，宿主恢复确保任务连续性。建议在生产环境定期运行 dataCheck，结合快照与 JSONL 追加，建立完善的备份与回滚策略。
