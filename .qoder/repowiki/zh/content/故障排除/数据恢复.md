...
- [migrate-v1.mjs](file://scripts/migrate-v1.mjs)
...
J["导出脚本<br/>scripts/export-db.mjs"] --> D
K["迁移存根<br/>scripts/migrate-v1.mjs"] --> I
```
...
- [migrate-v1.mjs:1-26](file://scripts/migrate-v1.mjs#L1-L26)
...
- 旧库导出：
  - scripts/export-db.mjs 将 SQLite 的 journal/practice 表导出为 JSONL 追加到 state。
...
Config --> Export["旧库导出(export-db.mjs)"]
Export --> Rollback{"需要回滚?"}
...
### 手动修复工具与脚本
- export-db.mjs：将旧 SQLite 的 journal/practice 表导出为 JSONL 追加到 state。
  - 用法：node scripts/export-db.mjs <vault路径>
  - 行为：若不存在旧库则退出；否则追加写入，不覆盖现有行。
- migrate-v1.mjs：退役存根，说明 v1→v2 断裂归档与手工迁移四步（仅参考，实际已归档至存档区）。
...
## 附录
- 常用命令与路径：
  - 导出旧库：node scripts/export-db.mjs <vault路径>
