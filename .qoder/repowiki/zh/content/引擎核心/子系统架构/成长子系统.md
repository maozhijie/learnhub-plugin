...
## 更新摘要
**变更内容**
- 新增卡点自报（stuck report）系统的完整集成说明
- 更新教练回合工作流以支持自报触发和消费标记机制
- 扩展频控系统和数据持久化层
- 增强故障排查指南以涵盖自报相关场景
...
- **新增**：卡点自报系统作为 t=0 的一等传感器，提供即时受阻信号捕获和处理。
...
- **新增**：stuck-report.ts：卡点自报纯函数层，提供频控、消费态折叠和注入块生成。
- **新增**：store.ts：扩展了自报追加、消费标记和流式读取功能。
- **新增**：jobs.ts：宿主编排层集成自报处理和消费标记逻辑。
...
## 核心组件
- GrowthSubsystem：统一编排罗盘读写、教练回合上下文包、生长批受理（轻量→全量→仲裁→回灌重裁）、边实验与复诊结算、就绪深度检查、ETA 折叠与路线对账。
...
- **新增**：StuckReport：卡点自报纯函数层，提供频控、消费态折叠和教练回合注入块生成。
...
- **新增**：自报层（StuckReport）：t=0 时刻的即时受阻信号捕获，通过频控和消费标记机制集成到教练回合工作流。
...
participant SR as "StuckReport"
participant ST as "Store"
Note over UI,ST : 自报触发流程
UI->>GS : stuckReportAppend(course, node, text)
GS->>SR : stuckReportGate(prior, node, today)
SR-->>GS : {ok : true/false}
alt 频控通过
GS->>ST : appendStuckReport(rec)
ST-->>GS : StuckReportRec
GS-->>UI : 返回自报记录
else 频控拒绝
GS-->>UI : 抛出错误带原因
end
Note over UI,ST : 教练回合消费流程
UI->>GS : coachGrowthBatch(course, agent)
GS->>GS : stuckPending() 获取待消费自报
GS->>SR : stuckReportInject(pending) 生成注入块
GS->>CR : 组装上下文包含自报注入
...
PR-->>GS : 应用结果
GS->>ST : appendStuckConsumption(targets) 消费标记
GS-->>UI : 返回裁决产物与轨迹
```
...
- [stuck-report.ts:26-83](file://src/engine/stuck-report.ts#L26-L83)
- [store.ts:148-181](file://src/engine/store.ts#L148-L181)
- [jobs.ts:648-701](file://src/host/jobs.ts#L648-L701)
...
- **新增**：自报集成：stuckReportAppend 验证节点存在性和频控，stuckPending 获取待消费自报，stuckMarkConsumed 幂等消费标记。
...
Pack --> Inject["stuckPending() 获取待消费自报"]
Inject --> Loop["agentLoop 三段式裁决"]
Loop --> Gate{"propose-edit 过门?"}
Gate --> |否| Repair["回灌重裁(仅一次)"]
Repair --> Gate
Gate --> |是| Apply["apply-edit(写入单元)"]
Apply --> Consume["stuckMarkConsumed() 消费标记"]
Consume --> End(["返回 applied 与轨迹"])
```

**图表来源**
- [growth-subsystem.ts:622-808](file://src/engine/growth-subsystem.ts#L622-L808)
- [growth-subsystem.ts:382-415](file://src/engine/growth-subsystem.ts#L382-L415)
- [proposals.ts:556-605](file://src/engine/proposals.ts#L556-605)
...
### 卡点自报系统 stuck-report：t=0 一等传感器
- **自由文本唯一录入形态**：原话逐字落账、零结构化，作为事实源避免提前提炼损耗。
- **频控系统**：同节点每学习日1条 + 全课程每日总量上限，防止回合风暴。
- **消费标记机制**：冲正式记录（append-only 凭证），读侧折叠出消费态，原记录永不改写。
- **教练回合注入**：将待消费自报以原文形式注入教练回合上下文，供归因分析使用。
- **幂等消费**：已消费的行 id 再标不计，重复标记无害，异常时自报自然留账不丢。

```mermaid
flowchart TD
A["stuckReportAppend(course, node, text)"] --> B["验证：节点在图上？"]
B --> |否| E["fail loud：节点不在图上"]
B --> |是| C["stuckReportGate(prior, node, today)"]
C --> D{"频控通过？"}
D --> |否| F["fail loud：{reason}"]
D --> |是| G["appendStuckReport({course, node, text})"]
G --> H["生成id: ts|node#{n}"]
H --> I["追加到practice流水"]
I --> J["返回StuckReportRec"]
```

**图表来源**
- [stuck-report.ts:26-50](file://src/engine/stuck-report.ts#L26-L50)
- [store.ts:148-162](file://src/engine/store.ts#L148-L162)
...
- [stuck-report.ts:15-16](file://src/engine/stuck-report.ts#L15-L16)
...
- **新增**：自报频控：同节点每学习日1条 + 全课程每日总量上限，防止回合风暴。
- **新增**：消费标记幂等：已消费的行 id 再标不计，避免重复处理开销。
...
- **新增**：自报频控拒绝：检查是否同节点同日已有自报，或全课程当日已达总量上限。
- **新增**：自报节点不存在：确认节点在当前课程的图上，卡点自报必须挂在学习者正在读的节点。
- **新增**：消费标记失败：检查 targets 是否为有效的自报行 id，幂等消费会忽略已消费记录。
...
- [growth-subsystem.ts:382-397](file://src/engine/growth-subsystem.ts#L382-L397)
- [proposals.ts:556-605](file://src/engine/proposals.ts#L556-L605)
- [probation.ts:76-95](file://src/engine/probation.ts#L76-L95)
- [coach-tools.ts:251-297](file://src/engine/coach-tools.ts#L251-L297)
- [stuck-report.ts:34-50](file://src/engine/stuck-report.ts#L34-L50)

## 结论
成长子系统以 GrowthSubsystem 为中心，将罗盘、教练回合、边实验与复诊、图分析有机整合，形成从数据采集、诊断到决策与执行的闭环。**新增的卡点自报系统**作为 t=0 的一等传感器，提供了即时受阻信号捕获和处理能力，通过频控和消费标记机制无缝集成到教练回合工作流中。通过严格的提案与账本机制、只读工具白名单与三段式裁决，系统在灵活性与安全性之间取得平衡。未来可通过扩展行为摘要指标、新增只读工具与分析维度，持续增强个性化指导能力。
...
### 集成卡点自报系统
- **自报提交**：调用 stuckReportAppend 验证节点存在性和频控，原话逐字落账。
- **待消费查询**：使用 stuckPending 获取未消费的自报列表，支持消费态折叠。
- **消费标记**：通过 stuckMarkConsumed 幂等标记已消费的自报行 id。
- **教练回合注入**：在 coachGrowthBatch 中自动注入待消费自报到上下文包。
- **频控配置**：通过 params.ts 中的 STUCK_REPORT_PER_NODE_PER_DAY 和 STUCK_REPORT_DAILY_COURSE_CAP 调整限制。

**章节来源**
- [growth-subsystem.ts:382-415](file://src/engine/growth-subsystem.ts#L382-L415)
- [stuck-report.ts:26-83](file://src/engine/stuck-report.ts#L26-L83)
- [store.ts:148-181](file://src/engine/store.ts#L148-L181)
- [params.ts:60-63](file://src/engine/params.ts#L60-L63)
- [jobs.ts:648-701](file://src/host/jobs.ts#L648-L701)