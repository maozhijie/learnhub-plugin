...
## 更新摘要
**变更内容**
- 移除了对已删除的 `export-db.mjs` 导出脚本的所有引用
- 更新了自动化运维脚本部分，移除旧库数据导出的使用说明
- 更新了备份恢复与数据迁移章节，反映当前无 SQLite 导出工具的状态
- 调整了故障排查指南中的相关说明
...
## 项目结构
围绕监控与运维的关键代码分布在宿主层（HTTP 路由、运行时、任务队列）、引擎层（健康分、存储、调度子系统）以及脚本层（冒烟测试、扫描工具）。
...
subgraph "脚本层"
G["gen-smoke.mjs<br/>冒烟测试驱动"]
SH["scan-host-state.mjs<br/>状态扫描"]
end
H --> R
H --> J
J --> C
...
- [scan-host-state.mjs:1-55](file://scripts/scan-host-state.mjs#L1-L55)
...
- 数据迁移与版本控制
  - schema 版本硬门保障升级安全；通过 learnhub.json 中的 schema.version 进行版本校验。
...
- 数据存储：系统使用 JSONL 格式替代 SQLite，无需数据库导出工具。
...
- 版本迁移：若 schema 版本不匹配，按错误提示运行迁移脚本；系统使用 JSONL 存储，无需数据库导出工具。
...
### 自动化运维脚本与告警规则配置
- 冒烟测试：scripts/gen-smoke.mjs 用于全管线冒烟测试，建议在部署前执行验证。
- 状态扫描：scripts/scan-host-state.mjs 用于检测宿主模块级可变状态，纳入 CI 门禁。
- 告警规则建议：
