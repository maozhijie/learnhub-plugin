...
## 更新摘要
**变更内容**
- 移除了种子提案工作流相关的所有UI组件和功能
- 更新了提案类型定义，移除 'seed' 选项
- 简化了提案页面，专注于编辑和富化提案处理
- 保留了历史种子提案的只读展示能力
...
该页面是面板侧的人审入口：所有由 agent/教练回合/工具面产生的图变更、富化回填、项目计划与里程碑产物等，均汇聚为"待审提案"，经人工确认后应用或拒绝，并全留痕。

**重要更新**：自 #256 起，种子提案工作流已退役，提案页面现专注于编辑和富化提案的处理。历史种子提案仍可读取但不再参与新的工作流程。

## 项目结构
- 前端页面：位于 ui/src/pages/ProposalsPage.tsx，负责展示提案列表、轮询刷新、应用/拒绝交互、结果跳转
- 前端 API：位于 ui/src/api.ts，封装 /learnhub/api/* 请求，提供 proposals、proposalApply、proposalReject 等接口
- 引擎层：位于 src/engine/proposals.ts，实现提案受理、校验、应用、拒绝、清单查询等核心逻辑
...
**图表来源**
- [ProposalsPage.tsx:1-151](file://ui/src/pages/ProposalsPage.tsx#L1-L151)
...
- [types.ts:247-252](file://src/engine/types.ts#L247-L252)
...
- 引擎提案域：受理 edit/enrich/project_plan/project_milestone/experiment 提案；执行多道门禁；应用时写入单元落盘；支持拒绝联动
- 存储与路径：提案元数据 JSON 持久化；产物 YAML 落盘；路径含自增 id，避免空态窗口
- 视图类型：统一前后端对提案结果与影响预览的结构约定
- 宿主任务：项目计划/里程碑生成走任务化流程，产出提案进入人审队列

**章节来源**
- [ProposalsPage.tsx:40-151](file://ui/src/pages/ProposalsPage.tsx#L40-L151)
- [proposals.ts（引擎）:559-605](file://src/engine/proposals.ts#L559-L605)
- [store.ts:213-228](file://src/engine/store.ts#L213-L228)
- [proposals.ts（视图类型）:7-58](file://src/engine/views/proposals.ts#L7-L58)
...
**图表来源**
- [ProposalsPage.tsx:68-111](file://ui/src/pages/ProposalsPage.tsx#L68-L111)
...
**更新**：移除了种子提案的影响预览功能，现在所有提案类型都使用统一的确认流程。

**章节来源**
- [ProposalsPage.tsx:18-38](file://ui/src/pages/ProposalsPage.tsx#L18-L38)
- [ProposalsPage.tsx:40-151](file://ui/src/pages/ProposalsPage.tsx#L40-L151)
...
### 提案状态与种类
- 种类：edit、enrich、project_plan、project_milestone、experiment（种子提案已退役）
- 状态：pending、applied、rejected
- 记录：ProposalRec 包含 id、kind、course、status、summary、artifact、created、decided、decision_note、pair

**更新**：PROPOSAL_KINDS 常量已移除 'seed' 选项，种子提案仅保留历史数据的只读展示。

**章节来源**
- [types.ts:247-252](file://src/engine/types.ts#L247-L252)
- [types.ts:288-300](file://src/engine/types.ts#L288-L300)

## 依赖关系分析
- 页面依赖 api.ts 提供的 proposals/apply/reject 方法
...
**重要更新**：随着种子提案工作流的退役，页面现专注于编辑和富化提案的高效处理，简化了用户界面并提高了操作一致性。
...
- 审核：
  - 在提案页面查看列表，点击"应用"前可查看提案摘要
  - 拒绝会留痕并可联动拒绝同源提案
- 应用：
  - 应用成功后刷新全局，按类型跳转到图/题库/项目/洞察等结果页

**更新**：移除了种子提案的特殊处理流程，现在所有提案类型都遵循相同的审核和应用流程。
