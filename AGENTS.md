...
**提示词散文冻结（2026-09-18，用户令，人工冻结期）**：`src/engine/prompts/` 的**代码结构可以动**（拆文件、改导出、挪常量、改类型），但一切提示词常量字符串**内的文字**是人工修订面——Agent 不得增、删、改一字。Agent 对提示词的唯一合法动作是**给出具体到常量名、原文引文与替换文本的修改建议**，由用户亲手落笔。**在用户按建议完成提示词修订之前，全仓禁止 commit / push / merge**（本约束优先于并行会话章法的「自动合入 main」流程——未获用户当次解禁前，池位成果只提交到池位分支并停下报告）。解禁条件：用户明示「提示词修订完成」。
