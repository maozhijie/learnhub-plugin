...
**提示词散文冻结（2026-09-18，用户令，人工冻结期）**：`src/engine/prompts/` 的**代码结构可以动**（拆文件、改导出、挪常量、改类型），但一切提示词常量字符串**内的文字**是人工修订面——Agent 不得增、删、改一字。Agent 对提示词的唯一合法动作是**给出具体到常量名、原文引文与替换文本的修改建议**，由用户亲手落笔。**冻结卡的是落地的最后一步，不是工作期禁令**：在用户按建议完成提示词修订之前，**禁止 push / merge 进 main**（本约束优先于并行会话章法的「自动合入 main」流程——未获解禁前，池位成果只提交到池位分支并停下报告）；**池位分支的本地 commit 照常**，探索、写码、本地提交都不受冻结影响。**触发与解禁都只认用户一句话**，与「本地有没有提示词 diff」无关——工作树全干净也不等于修订完成；解禁条件 = 用户明示「提示词修订完成」。**波及面是全仓而非单个池位**：被卡的是「合回 main」这个全局动作，故多票会一起排队，后合并方按并行会话章法重跑过门。**为什么当初立成全仓而非只冻提示词目录**：修订期用户手改散文时工作树里会有半成品（史实：笔误与重复编辑残留），若此时 Agent 提交，`git add -A` 会把半成品散文一并带进历史。**散文与依赖它的代码必须同提交**（改提示词 = 改生产行为）。
